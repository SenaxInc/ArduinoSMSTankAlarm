...
## 5. Scientific Validation & Scholarly Context

The architectural recommendations above are grounded in decades of petroleum engineering literature and artificial lift research:

*   **True Power vs. Motor Current:** Research presented to the Society of Petroleum Engineers (SPE) and IEEE extensively documents the flaw in using simple current (Amps) for Pump-Off Control on AC induction motors. As a pump motor unloads during a fluid pound event, the power factor drops significantly. This causes the stator current to remain artificially high even when mechanical work has ceased. True Power (kW) integrates voltage, current, and power factor, yielding a linear and mathematically sound representation of the polished rod work. *(Ref: "Sensorless Pump-Off Control," IEEE Transactions on Industry Applications; various SPE papers on less-sensor artificial lift).*
*   **The Gibbs Wave Equation & Mathematical Gold Standards:** The foundational science of rod pump diagnostics was established by Dr. Sam G. Gibbs (1960s), who formulated the damped wave equation to calculate downhole pump conditions (the "downhole card") from surface load and position measurements. Because True Dynamometer systems are prohibitively expensive for margin-sensitive stripper wells, our proxy measurements (True Power and RPM transients) intentionally avoid solving the complex Gibbs equation on the Edge while still capturing the kinematic surface symptoms of pump-off. 
*   **Machine Learning for Pump-Off States:** Recent studies (2015–present) validate the proposed "TinyML" approach. Researchers have successfully applied Support Vector Machines (SVM), Random Forests, and neural networks to continuous time-series data (like motor power and instantaneous RPM). These studies prove that ML models can classify pump states—such as fluid pound, gas lock, and parted rods—without requiring operators to manually define threshold spikes for each individual well's unique mechanical geometry. *(Ref: SPE papers on Automated Dynamometer Card Classification and IoT Artificial Lift)*.