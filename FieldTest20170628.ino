...
//power up and read sensor - GSM shield uses pins 0,1,2,3,7 + 8 for mega, 10 for yun 
  ...
  settingtext_tanknumber=1;  //can this be a for statement?
          ...
          string_currentsettings +="\n";
          string_currentsettings +="Current Fluid Level:";    
          string_currentsettings +=readvalue_one;   //convert to feet and inches here
...
void receiveSETTINGS()
{
  ...
  while (sms.available())
  {
           ...
           else if(sms.peek() == 'C') //C = Constant
    {
          ...
          else if(sms.peek() == 'T') //T = time between reports 
    {
              ...
              else if(sms.peek() == 'H')
    {
          ...
          delay(1000);
                    ...
                    else if(sms.peek() == 'S') //Request for Currrent Settings for tank one would = S1
    {
          //extract text into string
          string_settingtext_raw = sms.readString();  
          
          //delete "S" from begining of string here
          string_settingtext_raw.remove(1,1);

          //READ NEXT DIGIT AFTER "S" TO ASSIGN TO TANK # 1-9
          settingtext_tanknumber = string_settingtext_raw.charAt(1);
          ...
          } //end while check for sms available
