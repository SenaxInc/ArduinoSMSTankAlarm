...
static void handleViewerSummary(JsonDocument &doc, double epoch) {
  ...
  if (arr) {
    for (JsonVariantConst item : arr) {
      ...
      strlcpy(rec.objectType, item["ot"] | item["objectType"] | "", sizeof(rec.objectType));
      strlcpy(rec.sensorType, item["st"] | item["sensorType"] | "", sizeof(rec.sensorType));
      strlcpy(rec.measurementUnit, item["mu"] | item["measurementUnit"] | "", sizeof(rec.measurementUnit));
      if (!item["d"].isNull() || !item["change24h"].isNull()) {
        rec.change24h = (item["d"] | item["change24h"]).as<float>();
