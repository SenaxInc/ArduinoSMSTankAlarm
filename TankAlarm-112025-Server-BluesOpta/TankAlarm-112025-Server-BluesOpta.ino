...
struct ClientMetadata {
  ...
  float cachedTemperatureF;  // Cached current temperature in Â°F
...
struct TankCalibration {
  ...
  // Temperature is normalized around a reference temperature (70Â°F) for numerical stability
  float learnedSlope;         // Learned inches per mA (replaces maxValue calculation)
  float learnedOffset;        // Learned offset in inches
  float learnedTempCoef;      // Temperature coefficient (inches per Â°F deviation from refTemp)
...
// Reference temperature for temperature compensation (Â°F)
// Using 70Â°F as a reasonable "room temperature" baseline
...
static void handleAlarm(JsonDocument &doc, double epoch) {
  ...
  if (strcmp(type, "clear") == 0 || isRecovery) {
    ...
    strlcpy(rec->alarmType, type, sizeof(rec->alarmType));
    // Log new alarm event (skip for system alarms â€” they don't have tank levels)
    if (!isSystemAlarm) {
      const char *siteName = doc["s"] | doc["site"] | "";
  ...
  bool smsEnabled = false;  // Default off â€” only send SMS when client explicitly requests (se=true)
  if (doc["se"]) {
...
static float convertMaToLevelWithTemp(const char *clientUid, uint8_t tankNumber, float mA, float currentTempF) {
  ...
  if (cal && cal->hasLearnedCalibration) {
    ...
    if (cal->hasTempCompensation && currentTempF > TEMPERATURE_UNAVAILABLE + 1.0f) {
      // Temperature coefficient is in inches per Â°F deviation from 70Â°F
...
struct CalibrationPoint {
  ...
  float temperature;    // X2: temperature in Â°F (or TEMPERATURE_UNAVAILABLE)
...
static void recalculateCalibration(TankCalibration *cal) {
  ...
  if (tempCount >= MIN_TEMP_ENTRIES_FOR_COMPENSATION) {
    // Check for temperature variation (at least 10Â°F range)
...
static void handleCalibrationPost(EthernetClient &client, const String &body) {
  ...
  if (temperatureF != TEMPERATURE_UNAVAILABLE) {
    ...
    Serial.println(F("Â°F (6-hour avg)"));
