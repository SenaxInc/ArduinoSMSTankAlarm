...
async function basicInit(){try{const r=await fetch('/api/clients?summary=1');const d=await r.json();if(d&&d.srv&&d.srv.pu)els.productUid.value=d.srv.pu;if(d&&d.srv&&d.srv.cf)els.clientFleet.value=d.srv.cf;window._siteClients=d;populateRelayTargets(d);}catch(e){}const urlUid=getQueryParam('uid');if(urlUid&&els.clientUid)els.clientUid.value=urlUid;const urlSite=getQueryParam('site');if(urlSite&&els.siteName)els.siteName.value=urlSite;if(urlUid){fetchClientConfig(urlUid);}else{addSensor();}}function populateRelayTargets(d){const dl=document.getElementById('relayTargetSuggestions');if(!dl||!d||!d.cs)return;const seen=new Set();d.cs.forEach(c=>{const uid=c.c||'';if(uid&&!seen.has(uid)){seen.add(uid);const opt=document.createElement('option');const label=c.n||c.s||'';opt.value=uid;opt.label=label?label+' ('+uid+')':uid;dl.appendChild(opt);}});}basicInit();
})();
...
async function togglePause(){if(!state.pinConfigured){}else if(!state.pin){const p=prompt('Enter admin PIN to toggle pause');if(!p)return;state.pin=p.trim();}const target=!state.paused;try{const res=await fetch('/api/pause',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({paused:target,pin:state.pin||''})});if(!res.ok){if(res.status===403){state.pin=token;state.pinConfigured=true;throw new Error('PIN required or invalid');}throw new Error(await res.text()||'Pause toggle failed');}const data=await res.json();state.paused=!!data.paused;renderPauseButton();showToast(state.paused?'Paused for maintenance':'Resumed');}catch(err){showToast(err.message||'Pause toggle failed',true);}}
async function refreshTank(clientUid){if(state.refreshing)return;state.refreshing=true;renderSites();try{const res=await fetch('/api/refresh',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({client:clientUid,pin:state.pin||token||''})});if(!res.ok)throw new Error(await res.text()||'Refresh failed');applyServerData(await res.json());showToast('Refreshed');}catch(err){showToast(err.message||'Refresh failed',true);}finally{state.refreshing=false;renderSites();}}
...
static void handleClientConfigGet(EthernetClient &client, const String &query) {
  ...
  // Build response: {"config": <parsed config JSON>}
  DynamicJsonDocument doc(strlen(snap->payload) + 512);
  DynamicJsonDocument cfgDoc(strlen(snap->payload) + 256);
  DeserializationError err = deserializeJson(cfgDoc, snap->payload);
  if (err) {
    respondStatus(client, 500, F("Stored config is invalid JSON"));
    return;
  }
  doc["config"] = cfgDoc.as<JsonObject>();
  respondJson(client, doc);
}
