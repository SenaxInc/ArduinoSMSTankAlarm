...
static uint32_t hashTankKey(const char *clientUid, uint8_t tankNumber) {
  ...
  return hash;
    if(state.pinConfigured){
      els.changePinBtn.textContent='Change Admin PIN';
      els.pinStatus.textContent='✓ PIN configured';
      els.pinBadge.classList.remove('hidden');
    }else{
      els.changePinBtn.textContent='Set Admin PIN';
      els.pinStatus.textContent='⚠ No PIN set';
      els.pinStatus.style.color='#f59e0b';
      els.pinBadge.classList.add('hidden');
    }
...
static void sendClientDataJson(EthernetClient &client) {
  ...
  doc["fwv"] = String(FIRMWARE_VERSION);
  doc["fwb"] = String(FIRMWARE_BUILD_DATE);
...
static void handleServerSettingsPost(EthernetClient &client, const String &body) {
  ...
  if (doc["server"]) {
    ...
    if (serverObj["productUid"]) {
      const char *newProductUid = serverObj["productUid"] | "";
      if (strcmp(gConfig.productUid, newProductUid) != 0) {
