...
window.updateMonitorFields=function(id){const card=document.getElementById(`sensor-${id}`);const type=card.querySelector('.monitor-type').value;const numField=card.querySelector('.tank-num-field');const numFieldLabel=numField.querySelector('span');const nameLabel=card.querySelector('.name-label');const heightLabel=card.querySelector('.height-label');const sensorTypeSelect=card.querySelector('.sensor-type');const pulsesPerRevField=card.querySelector('.pulses-per-rev-field');const contentsField=card.querySelector('.contents-field');const calCheckbox=card.querySelector('.calibration-enabled');if(type==='gas'){numField.style.display='none';nameLabel.textContent='System Name';heightLabel.textContent='Max Pressure';pulsesPerRevField.style.display='none';contentsField.style.display='flex';sensorTypeSelect.value='2';calCheckbox.checked=false;updatePinOptions(id);}else if(type==='rpm'){numField.style.display='flex';numFieldLabel.textContent='Engine Number';nameLabel.textContent='Engine Name';heightLabel.textContent='Max RPM';pulsesPerRevField.style.display='flex';contentsField.style.display='none';sensorTypeSelect.value='3';calCheckbox.checked=false;updatePinOptions(id);}else{numField.style.display='flex';numFieldLabel.textContent='Tank Number';nameLabel.textContent='Name';heightLabel.textContent='Height(in)';pulsesPerRevField.style.display='none';contentsField.style.display='flex';calCheckbox.checked=true;}updateSensorTypeFields(id);};
...
funct)HTML" R"HTML(ion formatLevel(value,unit){if(typeof value !== 'number' || !isFinite(value)|| value < 0){return '--';}if(!unit||unit==='inches'){const feet = Math.floor(value / 12);const remainingInches = value % 12;if(feet === 0){return `${remainingInches.toFixed(1)}"`;}return `${feet}' ${remainingInches.toFixed(1)}"`;}return `${value.toFixed(2)} ${unit}`;}function getTankUnit(clientUid,tankNum){const t=tanks.find(x=>x.client===clientUid&&x.tank===tankNum);if(t&&t.measurementUnit&&t.measurementUnit!=='inches')return t.measurementUnit;if(t&&t.objectType&&t.objectType!=='tank')return t.measurementUnit||'units';return 'inches';}let tanks = [];let calibrations = [];let calibrationLogs = [];async funct)HTML" R"HTML(ion loadTanks(){try{const response = await fetch('/api/tanks');if(!response.ok)throw new Error('Failed to load tanks');const data = await response.json();tanks = (data.tanks || []).map(t=>({client:t.c,tank:t.k,site:t.s,label:t.n,contents:t.cn||'',levelInches:t.l,sensorMa:t.ma||0,sensorType:t.st||'',objectType:t.ot||'tank',measurementUnit:t.mu||'inches',delta:t.d,previousEpoch:t.pe,alarmActive:t.a,alarmType:t.at,lastUpdate:t.u}));populateTankDropdowns();}catch(err){console.error('Error loading tanks:',err);showToast('Failed to load tank list',true);}}
funct)HTML" R"HTML(ion populateTankDropdowns(){const tankSelect = document.getElementById('tankSelect');const logTankFilter = document.getElementById('logTankFilter');tankSelect.innerHTML = '<option value="">-- Select a sensor --</option>';logTankFilter.innerHTML = '<option value="">All Sensors</option>';const uniqueTanks = new Map();tanks.forEach(t =>{const key = `${t.client}:${t.tank}`;if(!uniqueTanks.has(key)){uniqueTanks.set(key,{client:t.client,tank:t.tank,site:t.site,label:t.label || `Sensor ${t.tank}`,heightInches:t.heightInches || 0,levelInches:t.levelInches || 0,sensorMa:t.sensorMa || 0,lastUpdate:t.lastUpdate || 0,objectType:t.objectType||'tank',measurementUnit:t.measurementUnit||'inches'});}});uniqueTanks.forEach((tank,key)=>{const option = document.createElement('option');option.value = key;const typeTag=tank.objectType==='tank'?'':'['+tank.objectType.toUpperCase()+'] ';option.textContent = `${typeTag}${tank.site} - ${tank.label}(#${tank.tank})`;tankSelect.appendChild(option.cloneNode(true));logTankFilter.appendChild(option);});}function updateLevelInput(){const sel=document.getElementById('tankSelect').value;if(!sel){document.getElementById('levelInputTank').style.display='flex';document.getElementById('levelInputGeneric').style.display='none';return;}const [uid,tn]=sel.split(':');const tank=tanks.find(t=>t.client===uid&&t.tank===parseInt(tn));const ot=tank?tank.objectType:'tank';const mu=tank?tank.measurementUnit:'inches';if(ot==='tank'||mu==='inches'){document.getElementById('levelInputTank').style.display='flex';document.getElementById('levelInputGeneric').style.display='none';}else{document.getElementById('levelInputTank').style.display='none';document.getElementById('levelInputGeneric').style.display='block';document.getElementById('levelUnitLabel').textContent=mu.toUpperCase();}}document.getElementById('tankSelect').addEventListener('change',updateLevelInput);
...
static void sendUnloadSms(const UnloadLogEntry &entry) {
  ...
  snprintf(message, sizeof(message), 
           ...
           entry.siteName, entry.tankNumber, delivered, u,
