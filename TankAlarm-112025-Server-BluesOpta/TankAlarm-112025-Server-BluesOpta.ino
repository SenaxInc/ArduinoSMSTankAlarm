...
struct ClientMetadata {
  ...
  float cachedTemperatureF;  // Cached current temperature in Ã‚Â°F
...
struct TankCalibration {
  ...
  // Temperature is normalized around a reference temperature (70Ã‚Â°F) for numerical stability
  float learnedSlope;         // Learned inches per mA (replaces maxValue calculation)
  float learnedOffset;        // Learned offset in inches
  float learnedTempCoef;      // Temperature coefficient (inches per Ã‚Â°F deviation from refTemp)
...
// Reference temperature for temperature compensation (Ã‚Â°F)
// Using 70Ã‚Â°F as a reasonable "room temperature" baseline
...
let pinMode='unlock';let pendingAction=null;const pinEls={modal:document.getElementById('pinModal'),form:document.getElementById('pinForm'),input:document.getElementById('pinInput'),cancel:document.getElementById('pinCancel')};function showPinModal(mode,cb){pinMode=mode;pendingAction=cb;if(pinEls.form)pinEls.form.reset();if(pinEls.modal)pinEls.modal.classList.remove('hidden');if(pinEls.input)pinEls.input.focus();}function hidePinModal(){if(pinEls.modal)pinEls.modal.classList.add('hidden');pendingAction=null;}function requestPin(cb){if(state.pinConfigured&&!state.pin){showPinModal('unlock',cb);}else{cb(state.pin);}}if(pinEls.form)pinEls.form.addEventListener('submit',async(e)=>{e.preventDefault();const p=pinEls.input.value.trim();if(p.length!==4){showToast('Invalid PIN',true);return;}state.pin=p;hidePinModal();if(pendingAction){pendingAction(p);}});if(pinEls.cancel)pinEls.cancel.addEventListener('click',hidePinModal);
const pauseBtn=document.getElementById('pauseBtn');function renderPauseBtn(){if(pauseBtn){pauseBtn.style.display=state.paused?'':'none';}}async function loadPauseState(){try{const r=await fetch('/api/clients?summary=1');const d=await r.json();if(d&&d.srv){state.paused=!!d.srv.ps;state.pinConfigured=!!d.srv.pc;renderPauseBtn();}}catch(e){}}if(pauseBtn)pauseBtn.addEventListener('click',()=>{requestPin(async(pin)=>{try{const r=await fetch('/api/pause',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({paused:false,pin:pin})});if(r.ok){state.paused=false;renderPauseBtn();showToast('Resumed');}else throw new Error('Failed');}catch(e){showToast('Error resuming',true);}});});loadPauseState();
const sensorTypes=[{value:0,label:'Digital Input'},{value:1,label:'Analog Input'},{value:2,label:'4-20mA'},{value:3,label:'RPM'}];const monitorTypes=[{value:'tank',label:'Tank Level'},{value:'gas',label:'Gas Pressure'},{value:'rpm',label:'RPM'}];let sensorCount=0;let inputIdCounter=0;window.updatePowerConfigInfo=function(){const ps=document.getElementById('powerSource').valu)HTML" R"HTML(e;const i=document.getElementById('powerConfigInfo');if(i)i.style.display=(ps==='solar_modbus_mppt')?'block':'none';};
function createSensorHtml(id){return `<div class="sensor-card" id="sensor-${id}"><div class="sensor-header"><span class="sensor-title">Sensor #${id+1}</span><button type="button" class="remove-btn" onclick="removeSensor(${id})">Remove</button></div><div class="form-grid"><label class="field"><span>Monitor Type</span><select class="monitor-type" onchange="updateMonitorFields(${id})">${monitorTypes.map(t=>`<option value="${t.value}">${t.label}</option>`).join('')}</select></label><label class="field tank-num-field"><span>Tank #</span><input type="number" class="tank-num" value="${id+1}"></label><label class="field"><span>Name</span><input type="text" class="tank-name"></label><label class="field contents-field"><span>Contents</span><input type="text" class="tank-contents"></label><label class="field"><span>Sensor Type</span><select class="sensor-type" onchange="updateFields(${id})">${sensorTypes.map(t=>`<option value="${t.value}">${t.label}</option>`).join('')}</select></label><label class="field"><span>Pin</span><select class="sensor-pin"><option value="0">I1</option><option value="1">I2</option><option value="2">I3</option><option value="3">I4</option><option value="4">I5</option><option value="5">I6</option><option value="6">I7</option><option value="7">I8</option></select></label><label class="field height-field"><span>Height(in)</span><input type="number" class="tank-height" value="120"></label></div><div class="collapsible-section alarm-section visible"><h4 style="margin:10px 0;">Alarms</h4><div class="form-grid"><label class="field"><input type="checkbox" class="high-alarm-enabled" checked> High Alarm <input type="number" class="high-alarm" value="100"></label><label class="field"><input type="checkbox" class="low-alarm-enabled" checked> Low Alarm <input type="number" class="low-alarm" value="20"></label></div></div><div class="collapsible-section sms-section" style="margin-top:10px;"><h4 style="margin:10px 0;">SMS Alerts</h4><div class="form-grid"><label class="field"><span>Phone Numbers</span><input type="text" class="sms-phones" placeholder="+1555..."></label></div></div></div>`;}
window.updateMonitorFields=function(id){/*Simplified*/};window.updateFields=function(id){/*Simplified*/};window.removeSensor=function(id){const e=document.getElementById(`sensor-${id}`);if(e)e.remove();};window.removeInput=function(id){const e=document.getElementById(`input-${id}`);if(e)e.remove();};
function addSensor(){const c=document.getElementById('sensorsContainer');const d=document.createElement('div');d.innerHTML=createSensorHtml(sensorCount++);c.appendChild(d.firstElementChild);}document.getElementById('addSensorBtn').addEventListener('click',addSensor);
function addInput(){const c=document.getElementById('inputsContainer');const d=document.createElement('div');d.className='sensor-card';d.id=`input-${inputIdCounter}`;d.innerHTML=`<div class="card-header"><h4>Input ${inputIdCounter+1}</h4><button type="button" class="remove-btn" onclick="removeInput(${inputIdCounter})">Remove</button></div><div class="form-grid"><label class="field">Name <input type="text" class="input-name" value="Clear Button"></label><label class="field">Pin <input type="number" class="input-pin" value="0"></label><label class="field">Mode <select class="input-mode"><option value="active_low">Active LOW</option><option value="active_high">Active HIGH</option></select></label><label class="field">Action <select class="input-action"><option value="clear_relays">Clear Alarms</option></select></label></div>`;c.appendChild(d);inputIdCounter++;}document.getElementById('addInputBtn').addEventListener('click',addInput);
function collectConfig(){const sMinutes=parseInt(document.getElementById('sampleMinutes').value)||30;const time=document.getElementById('reportTime').value.split(':');const ps=document.getElementById('powerSource').value;const cfg={productUid:document.getElementById('productUid').value.trim(),site:document.getElementById('siteName').value.trim(),deviceLabel:document.getElementById('deviceLabel').value.trim(),serverFleet:document.getElementById('serverFleet').value.trim(),sampleSeconds:sMinutes*60,reportHour:parseInt(time[0]||5),reportMinute:parseInt(time[1]||0),dailyEmail:document.getElementById('dailyEmail').value.trim(),powerSource:ps,solarPowered:ps.includes('solar'),mpptEnabled:ps.includes('mppt'),solarCharger:{enabled:ps==='solar_modbus_mppt'},tanks:[],clearButtonPin:-1,clearButtonActiveHigh:false};document.querySelectorAll('#sensorsContainer .sensor-card').forEach((c,i)=>{const type=parseInt(c.querySelector('.sensor-type').value);const pin=parseInt(c.querySelector('.sensor-pin').value);const tank={id:String.fromCharCode(65+i),number:parseInt(c.querySelector('.tank-num').value)||i+1,name:c.querySelector('.tank-name').value,contents:c.querySelector('.tank-contents').value,sensor:type===0?'digital':(type===2?'current':(type===3?'rpm':'analog')),primaryPin:parseInt(c.querySelector('.sensor-pin').value),maxValue:parseFloat(c.querySelector('.tank-height').value)||120,daily:true,upload:true,alarmSms:false};if(c.querySelector('.high-alarm-enabled').checked)tank.highAlarm=parseFloat(c.querySelector('.high-alarm').value);if(c.querySelector('.low-alarm-enabled').checked)tank.lowAlarm=parseFloat(c.querySelector('.low-alarm').value);const phones=c.querySelector('.sms-phones').value;if(phones)tank.smsAlert={phones:phones.split(','),trigger:'any',message:'Alarm'};if(tank.highAlarm||tank.lowAlarm||phones)tank.alarmSms=true;cfg.tanks.push(tank);});document.querySelectorAll('#inputsContainer .sensor-card').forEach(c=>{if(c.querySelector('.input-action').value==='clear_relays'){cfg.clearButtonPin=parseInt(c.querySelector('.input-pin').value);cfg.clearButtonActiveHigh=c.querySelector('.input-mode').value==='active_high';}});return cfg;}
async function submitConfig(e){e.preventDefault();requestPin(async(pin)=>{try{const clientUid=(els.clientUid&&els.clientUid.value?els.clientUid.value:'').trim();if(!clientUid){showToast('Device UID is required to send config',true);return;}const cfg=collectConfig();const res=await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pin:pin,client:clientUid,config:cfg})});if(res.ok){showToast('Configuration saved to device');}else{const t=await res.text();showToast('Error: '+t,true);}}catch(err){showToast('Error: '+err.message,true);}});}if(els.form)els.form.addEventListener('submit',submitConfig);
document.getElementById('downloadBtn').addEventListener('click',()=>{const cfg=collectConfig();const blob=new Blob([JSON.stringify(cfg,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='client_config.json';document.body.appendChild(a);a.click();document.body.removeChild(a);});
window.loadConfig=function(c){if(els.siteName)els.siteName.value=c.site||'';if(els.deviceLabel)els.deviceLabel.value=c.deviceLabel||c.label||'';if(els.productUid)els.productUid.value=c.productUid||'';if(els.sampleMinutes)els.sampleMinutes.value=(c.sampleSeconds||1800)/60;if(els.dailyEmail)els.dailyEmail.value=c.dailyEmail||'';const h=String(c.reportHour||5).padStart(2,'0');const m=String(c.reportMinute||0).padStart(2,'0');if(els.reportTime)els.reportTime.value=`${h}:${m}`;if(els.powerSource)els.powerSource.value=c.powerSource||'grid';updatePowerConfigInfo();document.getElementById('sensorsContainer').innerHTML='';sensorCount=0;if(c.tanks)c.tanks.forEach(t=>{addSensor();const card=document.getElementById(`sensor-${sensorCount-1}`);if(card){card.querySelector('.tank-num').value=t.number;card.querySelector('.tank-name').value=t.name||'';card.querySelector('.tank-contents').value=t.contents||'';card.querySelector('.tank-height').value=t.maxValue||120;card.querySelector('.sensor-type').value=t.sensor==='digital'?0:(t.sensor==='current'?2:(t.sensor==='rpm'?3:1));card.querySelector('.sensor-pin').value=t.primaryPin||0;if(t.highAlarm!==undefined)card.querySelector('.high-alarm').value=t.highAlarm;if(t.lowAlarm!==undefined)card.)HTML" R"HTML(querySelector('.low-alarm').value=t.lowAlarm;if(t.smsAlert&&t.smsAlert.phones)card.querySelector('.sms-phones').value=t.smsAlert.phones.join(',');}});showToast('Configuration loaded');};
...
async function basicInit(){try{const r=await fetch('/api/clients?summary=1');const d=await r.json();if(d&&d.srv&&d.srv.pu)els.productUid.value=d.srv.pu;}catch(e){}const urlUid=getQueryParam('uid');if(urlUid&&els.clientUid)els.clientUid.value=urlUid;addSensor();}basicInit();
})();
...
function updateClientDetails(uid){if(!uid){els.clientDetails.textContent='Select a client';return;}const client = lookupClient(uid);const detailParts = [];if(client){detailParts.push(`<strong>Site:</strong> ${escapeHtml(client.site || 'Unknown')}`);detailParts.push(`<strong>Latest Tank:</strong> ${escapeHtml(client.label || 'Tank')}#${escapeHtml(client.tank || '?')}at ${formatNumber(client.levelInches)}in`);}else{detailParts.push('Client not in telemetry data');}detailParts.push(`<strong>UID:</strong><code>${uid}</code>`);detailParts.push(`<a href="/config-generator?uid=${encodeURIComponent(uid)}">Edit Configuration</a>`);els.clientDetails.innerHTML = detailParts.join(' - ');}
...
function applyServerData(data,preferredUid){state.data=normalizeApiData(data);if(preferredUid){state.selected=preferredUid;}renderTelemetry();renderNewSites();populateClientSelect();}
...
static void handleAlarm(JsonDocument &doc, double epoch) {
  ...
  if (strcmp(type, "clear") == 0 || isRecovery) {
    ...
    strlcpy(rec->alarmType, type, sizeof(rec->alarmType));
    // Log new alarm event (skip for system alarms Ã¢â‚¬â€ they don't have tank levels)
    if (!isSystemAlarm) {
      const char *siteName = doc["s"] | doc["site"] | "";
  ...
  bool smsEnabled = false;  // Default off Ã¢â‚¬â€ only send SMS when client explicitly requests (se=true)
  if (doc["se"]) {
...
static float convertMaToLevelWithTemp(const char *clientUid, uint8_t tankNumber, float mA, float currentTempF) {
  ...
  if (cal && cal->hasLearnedCalibration) {
    ...
    if (cal->hasTempCompensation && currentTempF > TEMPERATURE_UNAVAILABLE + 1.0f) {
      // Temperature coefficient is in inches per Ã‚Â°F deviation from 70Ã‚Â°F
...
struct CalibrationPoint {
  ...
  float temperature;    // X2: temperature in Ã‚Â°F (or TEMPERATURE_UNAVAILABLE)
...
static void recalculateCalibration(TankCalibration *cal) {
  ...
  if (tempCount >= MIN_TEMP_ENTRIES_FOR_COMPENSATION) {
    // Check for temperature variation (at least 10Ã‚Â°F range)
  ...
  if (useTempCompensation) {
    ...
    if (fabs(det) < 0.0001f) {
      ...
      Serial.print(F(" in/Ã‚Â°F, offset="));
...
static void handleCalibrationPost(EthernetClient &client, const String &body) {
  ...
  if (temperatureF != TEMPERATURE_UNAVAILABLE) {
    ...
    Serial.println(F("Ã‚Â°F (6-hour avg)"));
