...
button{background:var(--primary);color:white;border:1px solid var(--primary);padding:0.55rem 1.05rem;border-radius:var(--radius);font-weight:500;cursor:pointer;font-size:0.95rem;transition:background 0.15s}
button:hover{background:var(--primary-hover)}
#loading-overlay{position:fixed;inset:0;background:rgba(255,255,255,0.8);backdrop-filter:blur(4px);z-index:9999;display:flex;align-items:center;justify-content:center;transition:opacity 0.3s}
...
R"HTML(const endpoint=kind==='restore'?'/api/ftp-restore':'/api/ftp-backup';try{const res=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!res.ok){const text=await res.text();throw new Error(text||`${kind} failed`);}const data=await res.json();if(data&&data.message){showToast(data.message,false);}else{showToast(kind==='restore'?'FTP restore completed':'FTP backup completed');}}catch(err){showToast(err.message||`FTP ${kind} failed`,true);}}if(els.ftpBackupNow)els.ftpBackupNow.addEventListener('click',()=>performFtpAction('backup'));if(els.ftpRestoreNow)els.ftpRestoreNow.addEventListener('click',()=>performFtpAction('restore'));const dfuEls={checkBtn:getEl('dfuCheckBtn'),enableBtn:getEl('dfuEnableBtn'),statusText:getEl('dfuStatusText'),fwVersion:getEl('fwVersionDisplay'),fwBuild:getEl('fwBuildDisplay'),voltage:getEl('serverVoltageDisplay'),serverTime:getEl('serverTimeDisplay'),fwLastUpdated:getEl('fwLastUpdatedDisplay')};async function loadDfuStatus(){try{const res=await fetch('/api/dfu/status');if(!res.ok)throw new Error('Failed to fetch DFU status');const data=await res.json();if(dfuEls.fwVersion)dfuEls.fwVersion.textContent='v'+data.currentVersion;if(dfuEls.fwBuild)dfuEls.fwBuild.textContent=data.buildDate||'Unknown';if(dfuEls.fwLastUpdated){if(data.buildDate){var updatedStr=data.buildDate;if(data.buildTime)updatedStr+=' '+data.buildTime;dfuEls.fwLastUpdated.textContent=updatedStr;}else{dfuEls.fwLastUpdated.textContent='Unknown';}}if(dfuEls.voltage){if(data.voltage>0){dfuEls.voltage.textContent=data.voltage.toFixed(2)+' V';}else{dfuEls.voltage.textContent='Not available';}}if(dfuEls.serverTime){if(data.serverTime>0){const d=new Date(data.serverTime*1000);dfuEls.serverTime.textContent=d.toLocaleString();}else{dfuEls.serverTime.textContent='Not synced';}}if(data.dfuInProgress){if(dfuEls.statusText)dfuEls.statusText.textContent='Firmware update in progress...';if(dfuEls.enableBtn)dfuEls.enableBtn.disabled=true;if(dfuEls.checkBtn)dfuEls.checkBtn.disabled=true;}else if(data.updateAvailable){if(dfuEls.statusText)dfuEls.statusText.textContent='Update available: v'+data.availableVersion;if(dfuEls.enableBtn)dfuEls.enableBtn.disabled=false;if(dfuEls.checkBtn)dfuEls.checkBtn.disabled=false;}else{if(dfuEls.statusText)dfuEls.statusText.textContent='Firmware is up to date';if(dfuEls.enableBtn)dfuEls.enableBtn.disabled=true;if(dfuEls.checkBtn)dfuEls.checkBtn.disabled=false;}}catch(err){if(dfuEls.statusText)dfuEls.statusText.textContent='Error: '+err.message;if(dfuEls.fwVersion)dfuEls.fwVersion.textContent='Unknown';if(dfuEls.fwBuild)dfuEls.fwBuild.textContent='Unknown';if(dfuEls.fwLastUpdated)dfuEls.fwLastUpdated.textContent='Unknown';if(dfuEls.voltage)dfuEls.voltage.textContent='Not available';if(dfuEls.serverTime)dfuEls.serverTime.textContent='Not synced';if(dfuEls.enableBtn)dfuEls.enableBtn.disabled=true;if(dfuEls.checkBtn)dfuEls.checkBtn.disabled=false;}}if(dfuEls.checkBtn)dfuEls.checkBtn.addEventListener('click',async()=>{dfuEls.checkBtn.disabled=true;if(dfuEls.statusText)dfuEls.statusText.textContent='Checking for updates...';await loadDfuStatus();});if(dfuEls.enableBtn)dfuEls.enableBtn.addEventListener('click',()=>{requestPin(async()=>{if(!confirm('Install firmware update? The server will restart after the update completes.'))return;try{const res=await fetch('/api/dfu/enable',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pin:state.pin||''})});if(!res.ok){const text=await res.text();throw new Error(text||'Failed to enable DFU');}showToast('Firmware update started - server will restart');if(dfuEls.enableBtn)dfuEls.enableBtn.disabled=true;if(dfuEls.statusText)dfuEls.statusText.textContent='Installing update...';}catch(err){showToast(err.message,true);}});});loadDfuStatus();loadSettings();}catch(e){console.error(e);const t=document.getElementById('toast');if(t){t.innerText='Script Error! '+e.message;t.style.background='red';t.classList.add('show');}}});</script></body></html>)HTML";
...
funct)HTML" R"HTML(ion formatLevel(inches){if(typeof inches !== 'number' || !isFinite(inches)|| inches < 0){return '--';}const feet = Math.floor(inches / 12);const remainingInches = inches % 12;if(feet === 0){return `${remainingInches.toFixed(1)}"`;}return `${feet}' ${remainingInches.toFixed(1)}"`;}let tanks = [];let calibrations = [];let calibrationLogs = [];async funct)HTML" R"HTML(ion loadTanks(){try{const response = await fetch('/api/tanks');if(!response.ok)throw new Error('Failed to load tanks');const data = await response.json();tanks = data.tanks || [];populateTankDropdowns();}catch(err){console.error('Error loading tanks:',err);showToast('Failed to load tank list',true);}}
...
static const char CLIENT_CONSOLE_HTML[] PROGMEM = R"HTML(<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Tank Alarm Client Console</title><link rel="stylesheet" href="/style.css"></head><body data-theme="light"><header><div class="bar"><div class="brand">TankAlarm</div><div class="header-actions"><button class="pause-btn" id="pauseBtn" aria-label="Resume data flow" style="display:none">Unpause</button><a class="pill secondary" href="/">Dashboard</a><a class="pill" href="/client-console">Client Console</a><a class="pill secondary" href="/contacts">Contacts</a><a class="pill secondary" href="/server-settings">Server Settings</a></div></div></header><main><section class="card"><h2 style="margin-top:0;">Remote Configuration</h2><p style="color:var(--muted);margin-bottom:12px;">Access remote update tools and generate new client configurations.</p><div class="actions"><a class="pill" href="#client-config">Remotely Update Config</a><a class="pill secondary" href="/config-generator">New Config Generator</a></div></section><section class="card"><h2 style="margin-top:0;">Calibration & Help</h2><p style="color:var(--muted);margin-bottom:12px;">Manage calibration entries and view setup guidance.</p><div class="actions"><a class="pill" href="/calibration">Open Calibration</a><a class="pill secondary" href="https://github.com/SenaxInc/ArduinoSMSTankAlarm/blob/master/Tutorials/Tutorials-112025/CLIENT_INSTALLATION_GUIDE.md" target="_blank" title="View Client Installation Guide">Help</a></div></section><div class="console-layout"><section class="card"><label class="field"><span>Select Client</span><select id="clientSelect"></select></label><div id="clientDetails" class="details">Select a client to review configuration.</div><div class="refresh-actions"><button type="button" id="refreshSelectedBtn">Refresh Selected Site</button><button type="button" class="secondary" id="refreshAllBtn">Refresh All Sites</button><button type="button" class="secondary" id="requestLocationBtn">Request GPS Location</button></div><div id="locationInfo" class="details" style="margin-top:8px;display:none;"></div></section><section class="card"><h2 style="margin-top:0;">New Sites (Unconfigured)</h2><p style="color:var(--muted);font-size:0.9rem;margin-bottom:12px;">Clients that need initial configuration. Click "Configure" to set up a new client remotely.</p><div class="actions" style="margin-bottom:12px;"><a class="pill secondary" href="https://github.com/SenaxInc/ArduinoSMSTankAlarm/blob/master/Tutorials/Tutorials-112025/CLIENT_INSTALLATION_GUIDE.md" target="_blank">📘 Quick Start Guide</a></div><div id="newSitesContainer" class="site-list"><div class="empty-state">No unconfigured clients detected.</div></div></section><section class="card"><h2 style="margin-top:0;">Active Sites</h2><div id="telemetryContainer" class="site-list"></div></section></div><section class="card" id="client-config"><h2 style="margin-top:0;">Client Configuration</h2><div id="configStatus" style="display:none;padding:12px;margin-bottom:16px;border-radius:var(--radius);background:#e6f2ff;border:1px solid #b3d9ff;color:#0066cc;"></div><form id="configForm"><div class="form-grid"><label class="field"><span>Product UID <span class="tooltip-icon" tabindex="0" data-tooltip="Blues Notehub Product UID. This is set fleet-wide in TankAlarm_Config.h and must match across all clients.">?</span></span><input id="productUidInput" type="text" readonly style="background:var(--chip);cursor:not-allowed;" placeholder="Loading..."></label><label class="field"><span>Site Name <span style="color:var(--danger);">*</span></span><input id="siteInpu)HTML" R"HTML(t" type="text" placeholder="Site name" required onchange="markUnsaved()"></label><label class="field"><span>Device Label <span style="color:var(--danger);">*</span></span><input id="deviceLabelInput" type="text" placeholder="Device label" required onchange="markUnsaved()"></label><label class="field"><span>Server Fleet</span><input id="routeInput" type="text" placeholder="tankalarm-server" onchange="markUnsaved()"></label><label class="field"><span>Sample Seconds</span><input id="sampleSecondsInput" type="number" min="30" step="30" onchange="markUnsaved()"></label><label class="field"><span>Level Change Threshold (in)</span><input id="levelChangeThresholdInput" type="number" min="0" step="0.1" placeholder="0 = disabled" onchange="markUnsaved()"></label><label class="field"><span>Report Hour (0-23)</span><input id="reportHourInput" type="number" min="0" max="23" onchange="markUnsaved()"></label><label class="field"><span>Report Minute (0-59)</span><input id="reportMinuteInput" type="number" min="0" max="59" onchange="markUnsaved()"></label><label class="field"><span>SMS Primary</span><input id="smsPrimaryInput" type="text" placeholder="+1234567890" onchange="markUnsaved()"></label><label class="field"><span>SMS Secondary</span><input id="smsSecondaryInput" type="text" placeholder="+1234567890" onchange="markUnsaved()"></label><label class="field"><span>Daily Report Email</span><input id="dailyEmailInput" type="email" placeholder="reports@example.com" onchange="markUnsaved()"></label></div><h3 style="margin-top:24px;">Power Configuration</h3><div class="form-grid"><label class="field"><span>Power Source<span class="tooltip-icon" tabindex="0" data-tooltip="Select the primary power source for this installation. Grid-Tied: Continuous AC power only. Grid + Battery Backup: AC power with UPS/battery backup. Solar + Battery: Off-grid solar with battery bank. Solar + MPPT: Basic MPPT charger without monitoring. Solar + Modbus MPPT: Includes Morningstar SunSaver MPPT monitoring via RS-485 for detailed battery/charging data.">?</span></span><select id="powerSourceInput" onchange="markUnsaved();updatePowerConfigInfoCC()"><option value="grid">Grid-Tied (AC Power Only)</option><option value="grid_battery">Grid-Tied + Battery Backup</option><option value="solar">Solar + Battery (Basic)</option><option value="solar_mppt">Solar + Battery + MPPT (No Monitor)</option><option value="solar_modbus_mppt">Solar + Modbus MPPT (RS-485 Monitor)</option></select></label></div><div id="powerConfigInfoCC" style="display:none;background:var(--chip);border:1px solid var(--card-border);padding:12px;margin-bottom:16px;font-size:0.9rem;color:var(--muted);"><strong>Hardware Requirement:</strong> Modbus MPPT requires the Arduino Opta with RS-485 expansion module for communication with the Morningstar SunSaver MPPT charge controller.</div><h3>Sensors</h3><div id="sensorsContainer"></div><div class="actions" style="margin-bottom: 24px;"><button type="button" id="addSensorBtn" class="secondary">+ Add Sensor</button></div><h3>Inputs (Buttons &amp; Switches)</h3><p style="color: var(--muted); font-size: 0.9rem; margin-bottom: 12px;">Configure physical inputs for actions like clearing relay alarms. More input actions will be available in future updates.</p><div id="inputsContainer"></div><div class="actions" style="margin-bottom: 24px;"><button type="button" id="addInputBtn" class="secondary">+ Add Input</button></div><div class="actions"><button type="submit" id="sendConfigBtn">Send Configuration</button><button type="button" id="downloadConfigBtn" class="secondary">Download JSON</button><button type="button" id="importConfigBtn" class="secondary">Import JSON</button></div></form></section></main><div id="toast"></div><div id="pinModal" class="modal hidden"><div class="modal-card"><div class="modal-badge" id="pinSessionBadge">Locked</div><h2 id="pinModalTitle">Enter Admin PIN</h2><p id="pinModalDescription">Enter the admin PIN to unlock configuration controls.</p><form id="pinForm"><label class="field"><span>PIN</span><input type="password" id="pinInput" inputmode="numeric" pattern="\d*" maxlength="4" autocomplete="off" required placeholder="4 digits" aria-describedby="pinHint" title="Enter exactly four digits (0-9)"><small class="pin-hint" id="pinHint">Enter your 4-digit PIN to unlock. PIN is set in Server Settings.</small></label><div class="actions"><button type="submit" id="pinSubmit">Unlock</button><button type="button" class="secondary" id="pinCancel">Cancel</button></div></form></div></div><input type="file" id="importFileInput" accept=".json" style="display:none;" /><script>(funct)HTML" R"HTML(ion(){const token=localStorage.getItem('tankalarm_token');if(!token){window.location.href='/login?redirect='+encodeURIComponent(window.location.pathname);return;}
...
funct)HTML" R"HTML(ion clearStoredPin(){localStorage.removeItem(PIN_STORAGE_KEY);}const state ={data:null,selected:null,originalConfig:null,hasUnsavedChanges:false,configHistory:[]};
const pinState ={value:token||null,configured:false,mode:'unlock'};
const els ={serverName:document.getElementById('serverName'),serverUid:document.getElementById('serverUid'),nextEmail:document.getElementById('nextEmail'),telemetryContainer:document.getElementById('telemetryContainer'),newSitesContainer:document.getElementById('newSitesContainer'),clientSelect:document.getElementById('clientSelect'),clientDetails:document.getElementById('clientDetails'),productUid:document.getElementById('productUidInput'),site:document.getElementById('siteInput'),deviceLabel:document.getElementById('deviceLabelInput'),route:document.getElementById('routeInput'),sampleSeconds:document.getElementById('sampleSecondsInput'),levelChangeThreshold:document.getElementById('levelChangeThresholdInput'),reportHour:document.getElementById('reportHourInput'),reportMinute:document.getElementById('reportMinuteInput'),smsPrimary:document.getElementById('smsPrimaryInput'),smsSecondary:document.getElementById('smsSecondaryInput'),dailyEmail:document.getElementById('dailyEmailInput'),powerSource:document.getElementById('powerSourceInput'),sensorsContainer:document.getElementById('sensorsContainer'),inputsContainer:document.getElementById('inputsContainer'),configStatus:document.getElementById('configStatus'),toast:document.getElementById('toast'),addSensor:document.getElementById('addSensorBtn'),addInput:document.getElementById('addInputBtn'),sendConfigBtn:document.getElementById('sendConfigBtn'),downloadConfigBtn:document.getElementById('downloadConfigBtn'),importConfigBtn:document.getElementById('importConfigBtn'),importFileInput:document.getElementById('importFileInput'),form:document.getElementById('configForm'),refreshSelectedBtn:document.getElementById('refreshSelectedBtn'),refreshAllBtn:document.getElementById('refreshAllBtn'),requestLocationBtn:document.getElementById('requestLocationBtn'),locationInfo:document.getElementById('locationInfo')};
const pinEls ={modal:document.getElementById('pinModal'),title:document.getElementById('pinModalTitle'),description:document.getElementById('pinModalDescription'),pin:document.getElementById('pinInput'),form:document.getElementById('pinForm'),submit:document.getElementById('pinSubmit'),cancel:document.getElementById('pinCancel')};funct)HTML" R"HTML(ion setFormDisabled(disabled){const controls = els.form.querySelectorAll('input,select,button');controls.forEach(control =>{if(control.dataset && control.dataset.pinControl === 'true'){return;}control.disabled = disabled;});if(els.addSensor)els.addSensor.disabled = disabled;if(els.addInput)els.addInput.disabled = disabled;}
...
funct)HTML" R"HTML(ion resetPinForm(){pinEls.form.reset();}
funct)HTML" R"HTML(ion showPinModal(){pinState.mode = 'unlock';resetPinForm();pinEls.modal.classList.remove('hidden');setFormDisabled(true);setTimeout(()=> pinEls.pin.focus(),50);}
funct)HTML" R"HTML(ion hidePinModal(){if(isPinModalVisible()){pinEls.modal.classList.add('hidden');resetPinForm();}}
async funct)HTML" R"HTML(ion requestPin(payload){const res = await fetch('/api/pin',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!res.ok){const text = await res.text();const error = new Error(text || 'PIN request failed');error.serverRejected = true;throw error;}const data = await res.json();pinState.configured = !!data.pinConfigured;return data;}
async funct)HTML" R"HTML(ion handlePinSubmit(event){event.preventDefault();let payload;let pinToStore = null;try{const pin = pinEls.pin.value.trim();if(!isFourDigits(pin)){throw new Error('PIN must be exactly 4 digits.');}payload ={pin};pinToStore = pin;const result = await requestPin(payload);pinState.value = pinToStore;storePin(pinToStore);hidePinModal();setFormDisabled(false);showToast((result && result.message)|| 'PIN verified');updatePinLock();}catch(err){if(err.serverRejected){invalidatePin();}showToast(err.message || 'PIN action failed',true);}}
funct)HTML" R"HTML(ion isFourDigits(value){return /^\d{4}$/.test(value || '');}
funct)HTML" R"HTML(ion updatePinLock(){const configured = !!(state.data && state.data.server && state.data.server.pinConfigured);pinState.configured = configured;if(!configured){invalidatePin();showToast('PIN not configured. Go to Server Settings.', true);return;}if(!pinState.value){if(!isPinModalVisible()){showPinModal();}}else{storePin(pinState.value);setFormDisabled(false);hidePinModal();}}setFormDisabled(true);
funct)HTML" R"HTML(ion showToast(message,isError){if(els.toast)els.toast.textContent= message;if(els.toast)els.toast.style.background = isError ? '#dc2626':'#0284c7';if(els.toast)els.toast.classList.add('show');setTimeout(()=>{if(els.toast)els.toast.classList.remove('show')},2500);}
funct)HTML" R"HTML(ion formatNumber(value){return(typeof value === 'number' && isFinite(value))? value.toFixed(1):'--';}
funct)HTML" R"HTML(ion valueOr(value,fallback){return(value === undefined || value === null)? fallback:value;}
funct)HTML" R"HTML(ion syncServerSettings(serverInfo){els.smsPrimary.value = valueOr(serverInfo && serverInfo.smsPrimary,'');els.smsSecondary.value = valueOr(serverInfo && serverInfo.smsSecondary,'');}
funct)HTML" R"HTML(ion formatEpoch(epoch){if(!epoch)return '--';const date = new Date(epoch * 1000);if(isNaN(date.getTime()))return '--';return date.toLocaleString(undefined,{year:'numeric',month:'numeric',day:'numeric',hour:'numeric',minute:'2-digit',hour12:true});}
funct)HTML" R"HTML(ion rowHtml(value){return(value === undefined || value === null || value === '')? '--':value;}
...
funct)HTML" R"HTML(ion sensorKeyFromValue(value){switch(value){case 0:return 'digital';case 2:return 'current';case 3:return 'rpm';default:return 'analog';}}
funct)HTML" R"HTML(ion renderTelemetry(){if(!els.telemetryContainer)return;els.telemetryContainer.innerHTML = '';if(!state.data || !state.data.clients)return;const sites ={};state.data.clients.forEach(client =>{const siteName = client.site || 'Unknown Site';if(!sites[siteName])sites[siteName]=[];sites[siteName].push(client);});Object.keys(sites).sort().forEach(siteName =>{const tanks = sites[siteName];const siteCard = document.createElement('div');siteCard.className = 'site-card';const header = document.createElement('div');header.className = 'site-header';header.innerHTML = `<span>${escapeHtml(siteName)}</span><span style="font-size:0.8rem;font-weight:400;color:var(--muted)">${tanks.length} Tank${tanks.length===1?'':'s'}</span>`;siteCard.appendChild(header);const grid = document.createElement('div');grid.className = 'site-tanks';tanks.forEach(tank =>{const tankDiv = document.createElement('div');tankDiv.className = 'site-tank';if(tank.alarm)tankDiv.classList.add('alarm');tankDiv.innerHTML = `<div class="tank-meta"><strong>${escapeHtml(tank.label || 'Tank')} #${tank.tank}</strong><code>${escapeHtml(tank.client).substring(0,8)}...</code></div><div class="tank-value">${formatNumber(tank.levelInches)} <small style="font-size:0.7em;color:var(--muted)">in</small></div><div class="tank-footer">${tank.alarm ? `<span style="color:var(--danger);font-weight:600">ALARM: ${escapeHtml(tank.alarmType)}</span>`:'Normal'}<span style="float:right">${formatEpoch(tank.lastUpdate)}</span></div>`;grid.appendChild(tankDiv);});siteCard.appendChild(grid);els.telemetryContainer.appendChild(siteCard);});}
funct)HTML" R"HTML(ion renderNewSites(){if(!els.newSitesContainer)return;els.newSitesContainer.innerHTML='';if(!state.data||!state.data.clients){els.newSitesContainer.innerHTML='<div class="empty-state">No clients detected yet.</div>';return;}const unconfigured=state.data.clients.filter(client=>{const hasNoSite=!client.site||client.site.trim()===''||client.site==='Unknown Site';const hasNoLabel=!client.label||client.label.trim()==='';const hasDefaultLabel=client.label&&(client.label==='Client-112025'||client.label.includes('Tank A'));return hasNoSite||(hasNoLabel||hasDefaultLabel);});if(unconfigured.length===0){els.newSitesContainer.innerHTML='<div class="empty-state">No unconfigured clients detected.</div>';return;}unconfigured.forEach(client=>{const card=document.createElement('div');card.className='site-card';card.style.borderLeft='4px solid var(--warning)';const firmwareVersion=client.firmwareVersion||client.version||'unknown';const lastSeenEpoch=client.lastUpdate||0;const now=Date.now()/1000;const ageSeconds=now-lastSeenEpoch;let lastSeenDisplay='';if(ageSeconds<120){lastSeenDisplay='<span style="color:#10b981;">● Active now</span>';}else if(ageSeconds<3600){const mins=Math.floor(ageSeconds/60);lastSeenDisplay=`Last seen ${mins} min${mins!==1?'s':''} ago`;}else if(ageSeconds<86400){const hours=Math.floor(ageSeconds/3600);lastSeenDisplay=`Last seen ${hours} hour${hours!==1?'s':''} ago`;}else{const days=Math.floor(ageSeconds/86400);lastSeenDisplay=`Last seen ${days} day${days!==1?'s':''} ago - ${formatEpoch(lastSeenEpoch)}`;}card.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;"><div><strong style="color:var(--warning)">⚠ Unconfigured Client</strong><div style="font-size:0.85rem;color:var(--muted);margin-top:4px;">UID: <code>${escapeHtml(client.client)}</code></div><div style="font-size:0.85rem;margin-top:4px;">${lastSeenDisplay}</div><div style="font-size:0.85rem;margin-top:4px;color:var(--muted);">Firmware: ${escapeHtml(firmwareVersion)}</div></div><button type="button" class="configure-btn" data-uid="${escapeHtml(client.client)}" style="white-space:nowrap;">Configure →</button></div>`;card.querySelector('.configure-btn').addEventListener('click',()=>{state.selected=client.client;els.clientSelect.value=client.client;loadConfigIntoForm(client.client);document.getElementById('client-config').scrollIntoView({behavior:'smooth'});showToast('Configure this client below and click "Send Configuration"');});els.newSitesContainer.appendChild(card);});}
funct)HTML" R"HTML(ion ensureOption(uid,label){let option = Array.from(els.clientSelect.options).find(opt => opt.value === uid);if(!option){option = document.createElement('option');option.value = uid;els.clientSelect.appendChild(option);}option.textContent = label;}
funct)HTML" R"HTML(ion populateClientSelect(){els.clientSelect.innerHTML = '';if(!state.data || !state.data.clients)return;state.data.clients.forEach(client =>{const label = `${escapeHtml(client.site || 'Site')} - ${escapeHtml(client.label || 'Tank')}(#${client.tank || '?'})`;ensureOption(client.client,label);});if(state.data.configs){state.data.configs.forEach(entry =>{ensureOption(entry.client,`${entry.site || 'Site'} - Stored config`);});}if(!state.selected && els.clientSelect.options.length){state.selected = els.clientSelect.options[0].value;}if(state.selected){els.clientSelect.value = state.selected;}}
funct)HTML" R"HTML(ion lookupClient(uid){if(!state.data || !state.data.clients)return null;return state.data.clients.find(c => c.client === uid)|| null;}
funct)HTML" R"HTML(ion lookupConfig(uid){if(!state.data || !state.data.configs)return null;const entry = state.data.configs.find(c => c.client === uid);if(!entry)return null;if(entry.config){return entry.config;}if(entry.configJson){try{entry.config = JSON.parse(entry.configJson);return entry.config;}catch(err){console.warn('Stored config failed to parse',err);}}return null;}
funct)HTML" R"HTML(ion buildDefaultConfig(uid){const client = lookupClient(uid);const serverDefaults =(state.data && state.data.server)? state.data.server:{};
const tankList =(client && Array.isArray(client.tanks))? client.tanks:[];const firstTank = tankList.length ? tankList[0]:null;const tankId = firstTank && firstTank.tank ? firstTank.tank:'A';const parsedNumber = firstTank && firstTank.tank ? parseInt(firstTank.tank,10):NaN;const defaultTank ={id:tankId || 'A',name:firstTank ?(firstTank.label || `Tank ${tankId || 'A'}`):'Tank A',contents:'',number:isFinite(parsedNumber)&& parsedNumber > 0 ? parsedNumber:1,sensor:'analog',primaryPin:0,secondaryPin:-1,loopChannel:-1,maxValue:firstTank && typeof firstTank.heightInches === 'number' ? firstTank.heightInches:120,levelChangeThreshold:0,hysteresis:2.0,highAlarm:100,lowAlarm:20,daily:true,alarmSms:true,upload:true};return{site:client ?(client.site || ''):'',deviceLabel:client ? `${((client.site || 'Client')).replace(/\s+/g,'-')}-${escapeHtml(client.tank || tankId || 'A')}`:'Client-112025',serverFleet:'tankalarm-server',sampleSeconds:1800,reportHour:5,reportMinute:0,dailyEmail:serverDefaults.dailyEmail || '',powerSource:'grid',solarPowered:false,mpptEnabled:false,clearButtonPin:-1,clearButtonActiveHigh:false,tanks:tankList.length ? tankList.map(t =>({id:t.tank || 'A',name:t.label ||(t.tank ? `Tank ${t.tank}`:'Tank'),contents:'',number:t.tank ?(parseInt(t.tank,10)|| 1):1,sensor:'analog',primaryPin:0,secondaryPin:-1,loopChannel:-1,maxValue:typeof t.heightInches === 'number' ? t.heightInches:120,levelChangeThreshold:0,hysteresis:2.0,highAlarm:100,lowAlarm:20,daily:true,alarmSms:true,upload:true})):[defaultTank]};}
funct)HTML" R"HTML(ion loadConfigIntoForm(uid){if(!uid)return;const client = lookupClient(uid);const stored = lookupConfig(uid);const config = stored ? JSON.parse(JSON.stringify(stored)):buildDefaultConfig(uid);state.selected = uid;state.originalConfig=JSON.parse(JSON.stringify(config));clearUnsaved();if(els.productUid){const productUid=config.productUid||(state.data&&state.data.server&&state.data.server.productUid)||'DEFAULT_PRODUCT_UID';els.productUid.value=productUid;}els.site.value = config.site || '';els.deviceLabel.value = config.deviceLabel || '';els.route.value = config.serverFleet || '';els.sampleSeconds.value = valueOr(config.sampleSeconds,1800);els.levelChangeThreshold.value = valueOr(config.levelChangeThreshold,0);els.reportHour.value = valueOr(config.reportHour,5);els.reportMinute.value = valueOr(config.reportMinute,0);els.dailyEmail.value = config.dailyEmail || '';if(els.powerSource){els.powerSource.value = config.powerSource || 'grid';}sensorCount=0;inputIdCounter=0;if(els.sensorsContainer){els.sensorsContainer.innerHTML='';}if(els.inputsContainer){els.inputsContainer.innerHTML='';}const isTrulyNewClient=!stored&&client&&(!client.site||client.site.trim()===''||client.site==='Unknown Site')&&(!client.label||client.label.trim()==='');if(config.tanks && config.tanks.length){config.tanks.forEach(tank=>{addSensorFromConfig(tank);});}else if(!isTrulyNewClient){addSensor();}if(config.clearButtonPin && config.clearButtonPin >= 0){addInputFromConfig({name:'Clear Button',pin:config.clearButtonPin,mode:config.clearButtonActiveHigh?'active_high':'active_low',action:'clear_relays'});}const detailParts = [];if(client){detailParts.push(`<strong>Site:</strong> ${escapeHtml(client.site || 'Unknown')}`);detailParts.push(`<strong>Latest Tank:</strong> ${escapeHtml(client.label || 'Tank')}#${escapeHtml(client.tank || '?')}at ${formatNumber(client.levelInches)}in`);}detailParts.push(`<strong>Target UID:</strong><code>${uid}</code>`);els.clientDetails.innerHTML = detailParts.join(' - ');}
window.addSensorFromConfig=funct)HTML" R"HTML(ion(tank){const id=sensorCount;const container=els.sensorsContainer;if(!container)return;const monitorType=tank.sensor==='rpm'?'rpm':(tank.contents&&tank.contents.toLowerCase().includes('gas')?'gas':'tank');const sensorTypeValue=tank.sensor==='digital'?0:(tank.sensor==='current'?2:(tank.sensor==='rpm'?3:1));const pin=tank.rpmPin>=0?tank.rpmPin:(tank.loopChannel>=0?tank.loopChannel:(tank.primaryPin||0));const div=document.createElement('div');div.innerHTML=`<div class="sensor-card" id="sensor-${id}"><div class="sensor-header"><span class="sensor-title">Sensor #${id+1}</span><button type="button" class="remove-btn" onclick="document.getElementById('sensor-${id}').remove()">Remove</button></div><div class="form-grid"><label class="field"><span>Monitor Type</span><select class="monitor-type" onchange="updateMonitorFields(${id})">${monitorTypes.map(t=>`<option value="${t.value}"${t.value===monitorType?' selected':''}>${t.label}</option>`).join('')}</select></label><label class="field tank-num-field"><span>${monitorType==='rpm'?'Engine':'Tank'} Number</span><input type="number" class="tank-num" value="${tank.number||1}"></label><label class="field"><span><span class="name-label">${monitorType==='gas'?'System Name':'Name'}</span></span><input type="text" class="tank-name" placeholder="Name" value="${escapeHtml(tank.name||'')}"></label><label class="field contents-field"${monitorType==='rpm'?' style="display:none;"':''}><span>Contents</span><input type="text" class="tank-contents" placeholder="e.g. Diesel, Water" value="${escapeHtml(tank.contents||'')}"></label><label class="field"><span>Sensor Type</span><select class="sensor-type" onchange="updatePinOptions(${id});updateSensorTypeFields(${id})">${sensorTypes.map(t=>`<option value="${t.value}"${t.value===sensorTypeValue?' selected':''}>${t.label}</option>`).join('')}</select></label><label class="field"><span>Pin / Channel</span><select class="sensor-pin">${(sensorTypeValue===2?expansionChannels:optaPins).map(p=>`<option value="${p.value}"${p.value===pin?' selected':''}>${p.label}</option>`).join('')}</select></label><label class="field switch-mode-field" style="${sensorTypeValue===0?'':'display:none;'}"><span>Switch Mode</span><select class="switch-mode"><option value="NO"${tank.digitalSwitchMode==='NO'?' selected':''}>Normally-Open(NO)</option><option value="NC"${tank.digitalSwitchMode==='NC'?' selected':''}>Normally-Closed(NC)</option></select></label><label class="field pulses-per-rev-field" style="${sensorTypeValue===3?'':'display:none;'}"><span>Pulses/Rev</span><input type="number" class="pulses-per-rev" value="${tank.pulsesPerRev||1}" min="1" max="255"></label><label class="field current-loop-type-field" style="${sensorTypeValue===2?'':'display:none;'}"><span>4-20mA Sensor Type</span><select class="current-loop-type" onchange="updateCurrentLoopFields(${id})"><option value="pressure"${tank.currentLoopType==='pressure'||!tank.currentLoopType?' selected':''}>Pressure Sensor(Bottom-Mounted)</option><option value="ultrasonic"${tank.currentLoopType==='ultrasonic'?' selected':''}>Ultrasonic Sensor(Top-Mounted)</option></select></label><label class="field height-field" style="${sensorTypeValue===0?'display:none;':''}"><span><span class="height-label">${monitorType==='gas'?'Max Pressure (PSI)':(monitorType==='rpm'?'Max RPM':'Height(in)')}</span></span><input type="number" class="tank-height" value="${tank.maxValue||120}"></label><label class="field level-change-field" style="${monitorType==='tank'&&sensorTypeValue!==0?'':'display:none;'}"><span>Level Change Threshold (in)</span><input type="number" class="level-change-threshold" step="0.1" value="${tank.levelChangeThreshold||0}" min="0"></label></div><button type="button" class="add-section-btn add-alarm-btn tertiary" onclick="toggleAlarmSection(${id})">+ Add Alarm</button><div class="collapsible-section alarm-section${tank.alarmSms||tank.highAlarm||tank.lowAlarm?' visible':''}"><h4 style="margin:16px 0 8px;font-size:0.95rem;"><span class="alarm-section-title">Alarm Thresholds</span><button type="button" class="remove-btn" onclick="removeAlarmSection(${id})" style="float:right;">Remove Alarm</button></h4><div class="form-grid alarm-thresholds-grid"${sensorTypeValue===0?' style="display:none;"':''}><div class="field"><span><label style="display:flex;align-items:center;gap:6px;"><input type="checkbox" class="high-alarm-enabled"${tank.highAlarm?' checked':''}> <span class="high-alarm-label">High Alarm (in)</span></label></span><input type="number" class="high-alarm" value="${tank.highAlarm||100}"></div><div class="field"><span><label style="display:flex;align-items:center;gap:6px;"><input type="checkbox" class="low-alarm-enabled"${tank.lowAlarm?' checked':''}> <span class="low-alarm-label">Low Alarm (in)</span></label></span><input type="number" class="low-alarm" value="${tank.lowAlarm||20}"></div></div><div class="form-grid digital-alarm-grid"${sensorTypeValue!==0?' style="display:none;"':''}><div class="field" style="grid-column:1 / -1;"><span>Trigger Condition</span><select class="digital-trigger-state"><option value="activated"${tank.digitalTrigger==='activated'?' selected':''}>When Switch is Activated(fluid detected)</option><option value="not_activated"${tank.digitalTrigger==='not_activated'?' selected':''}>When Switch is NOT Activated(no fluid)</option></select></div></div></div><button type="button" class="add-section-btn add-relay-btn${tank.alarmSms||tank.highAlarm||tank.lowAlarm?'':' hidden'} tertiary" onclick="toggleRelaySection(${id})">+ Add Relay Control</button><button type="button" class="add-section-btn add-sms-btn${tank.alarmSms||tank.highAlarm||tank.lowAlarm?'':' hidden'} tertiary" onclick="toggleSmsSection(${id})">+ Add SMS Alert</button></div>`;const card=div.firstElementChild;if(tank.relayTargetClient){const relaySection=document.createElement('div');relaySection.className='collapsible-section relay-section visible';relaySection.innerHTML=`<h4 style="margin:16px 0 8px;font-size:0.95rem;">Relay Control<button type="button" class="remove-btn" onclick="document.getElementById('sensor-${id}').querySelector('.relay-section').remove();document.getElementById('sensor-${id}').querySelector('.add-relay-btn').classList.remove('hidden')" style="float:right;">Remove Relay</button></h4><div class="form-grid"><label class="field"><span>Target Client UID</span><input type="text" class="relay-target" value="${escapeHtml(tank.relayTargetClient||'')}"></label><label class="field"><span>Trigger On</span><select class="relay-trigger"><option value="any"${tank.relayTrigger==='any'?' selected':''}>Any Alarm</option><option value="high"${tank.relayTrigger==='high'?' selected':''}>High Alarm Only</option><option value="low"${tank.relayTrigger==='low'?' selected':''}>Low Alarm Only</option></select></label><label class="field"><span>Relay Mode</span><select class="relay-mode" onchange="toggleRelayDurations(${id})"><option value="momentary"${tank.relayMode==='momentary'?' selected':''}>Momentary</option><option value="until_clear"${tank.relayMode==='until_clear'?' selected':''}>Until Alarm Clears</option><option value="manual_reset"${tank.relayMode==='manual_reset'?' selected':''}>Manual Reset</option></select></label><div class="field"><span>Relay Outputs</span><div style="display:flex;gap:12px;padding:8px 0;">${[1,2,3,4].map(i=>`<label><input type="checkbox" class="relay-${i}" value="${1<<(i-1)}"${(tank.relayMask&(1<<(i-1)))?'checked':''}> R${i}</label>`).join('')}</div></div></div>`;card.querySelector('.add-sms-btn').before(relaySection);card.querySelector('.add-relay-btn').classList.add('hidden');}if(tank.smsAlert&&tank.smsAlert.phones){const smsSection=document.createElement('div');smsSection.className='collapsible-section sms-section visible';smsSection.innerHTML=`<h4 style="margin:16px 0 8px;font-size:0.95rem;">SMS Alert<button type="button" class="remove-btn" onclick="document.getElementById('sensor-${id}').querySelector('.sms-section').remove();document.getElementById('sensor-${id}').querySelector('.add-sms-btn').classList.remove('hidden')" style="float:right;">Remove SMS</button></h4><div class="form-grid"><label class="field" style="grid-column:1 / -1;"><span>Phone Numbers</span><input type="text" class="sms-phones" value="${escapeHtml(tank.smsAlert.phones.join(','))}"></label><label class="field"><span>Trigger On</span><select class="sms-trigger"><option value="any"${tank.smsAlert.trigger==='any'?' selected':''}>Any Alarm</option><option value="high"${tank.smsAlert.trigger==='high'?' selected':''}>High Alarm Only</option><option value="low"${tank.smsAlert.trigger==='low'?' selected':''}>Low Alarm Only</option></select></label><label class="field" style="grid-column:span 2;"><span>Custom Message</span><input type="text" class="sms-message" value="${escapeHtml(tank.smsAlert.message||'')}"></label></div>`;card.appendChild(smsSection);card.querySelector('.add-sms-btn').classList.add('hidden');}container.appendChild(card);sensorCount++;};
window.addSensor=funct)HTML" R"HTML(ion(){const id=sensorCount;const container=els.sensorsContainer;if(!container)return;const div=document.createElement('div');div.innerHTML=`<div class="sensor-card" id="sensor-${id}"><div class="sensor-header"><span class="sensor-title">Sensor #${id+1}</span><button type="button" class="remove-btn" onclick="document.getElementById('sensor-${id}').remove()">Remove</button></div><div class="form-grid"><label class="field"><span>Monitor Type</span><select class="monitor-type" onchange="updateMonitorFields(${id})">${monitorTypes.map(t=>`<option value="${t.value}">${t.label}</option>`).join('')}</select></label><label class="field tank-num-field"><span>Tank Number</span><input type="number" class="tank-num" value="${id+1}"></label><label class="field"><span><span class="name-label">Name</span></span><input type="text" class="tank-name" placeholder="Name"></label><label class="field contents-field"><span>Contents</span><input type="text" class="tank-contents" placeholder="e.g. Diesel, Water"></label><label class="field"><span>Sensor Type</span><select class="sensor-type" onchange="updatePinOptions(${id});updateSensorTypeFields(${id})">${sensorTypes.map(t=>`<option value="${t.value}">${t.label}</option>`).join('')}</select></label><label class="field"><span>Pin / Channel</span><select class="sensor-pin">${optaPins.map(p=>`<option value="${p.value}">${p.label}</option>`).join('')}</select></label><label class="field switch-mode-field" style="display:none;"><span>Switch Mode</span><select class="switch-mode"><option value="NO">Normally-Open(NO)</option><option value="NC">Normally-Closed(NC)</option></select></label><label class="field pulses-per-rev-field" style="display:none;"><span>Pulses/Rev</span><input type="number" class="pulses-per-rev" value="1" min="1" max="255"></label><label class="field current-loop-type-field" style="display:none;"><span>4-20mA Sensor Type</span><select class="current-loop-type" onchange="updateCurrentLoopFields(${id})"><option value="pressure">Pressure Sensor(Bottom-Mounted)</option><option value="ultrasonic">Ultrasonic Sensor(Top-Mounted)</option></select></label><label class="field height-field"><span><span class="height-label">Height(in)</span></span><input type="number" class="tank-height" value="120"></label><label class="field level-change-field" style="display:none;"><span>Level Change Threshold (in)</span><input type="number" class="level-change-threshold" step="0.1" value="0" min="0"></label></div><button type="button" class="add-section-btn add-alarm-btn tertiary" onclick="toggleAlarmSection(${id})">+ Add Alarm</button><div class="collapsible-section alarm-section"><h4 style="margin:16px 0 8px;font-size:0.95rem;"><span class="alarm-section-title">Alarm Thresholds</span><button type="button" class="remove-btn" onclick="removeAlarmSection(${id})" style="float:right;">Remove Alarm</button></h4><div class="form-grid alarm-thresholds-grid"><div class="field"><span><label style="display:flex;align-items:center;gap:6px;"><input type="checkbox" class="high-alarm-enabled" checked> <span class="high-alarm-label">High Alarm (in)</span></label></span><input type="number" class="high-alarm" value="100"></div><div class="field"><span><label style="display:flex;align-items:center;gap:6px;"><input type="checkbox" class="low-alarm-enabled" checked> <span class="low-alarm-label">Low Alarm (in)</span></label></span><input type="number" class="low-alarm" value="20"></div></div><div class="form-grid digital-alarm-grid" style="display:none;"><div class="field" style="grid-column:1 / -1;"><span>Trigger Condition</span><select class="digital-trigger-state"><option value="activated">When Switch is Activated</option><option value="not_activated">When Switch is NOT Activated</option></select></div></div></div><button type="button" class="add-section-btn add-relay-btn hidden tertiary" onclick="toggleRelaySection(${id})">+ Add Relay Control</button><button type="button" class="add-section-btn add-sms-btn hidden tertiary" onclick="toggleSmsSection(${id})">+ Add SMS Alert</button></div>`;container.appendChild(div.firstElementChild);sensorCount++;};
window.addInputFromConfig=funct)HTML" R"HTML(ion(input){const id=inputIdCounter;const container=els.inputsContainer;if(!container)return;const card=document.createElement('div');card.className='sensor-card';card.id=`input-${id}`;card.innerHTML=`<div class="card-header" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;"><h4 style="margin:0;font-size:1rem;">Input ${id+1}</h4><button type="button" class="remove-btn" onclick="document.getElementById('input-${id}').remove()">Remove</button></div><div class="form-grid"><label class="field"><span>Input Name</span><input type="text" class="input-name" value="${escapeHtml(input.name||'Input')}"></label><label class="field"><span>Pin Number</span><input type="number" class="input-pin" value="${input.pin||0}" min="0" max="99"></label><label class="field"><span>Input Mode</span><select class="input-mode">${inputModes.map(m=>`<option value="${m.value}"${m.value===input.mode?' selected':''}>${m.label}</option>`).join('')}</select></label><label class="field"><span>Action</span><select class="input-action">${inputActions.map(a=>`<option value="${a.value}"${a.value===input.action?' selected':''}>${a.label}</option>`).join('')}</select></label></div>`;container.appendChild(card);inputIdCounter++;};
window.addInput=funct)HTML" R"HTML(ion(){const id=inputIdCounter;const container=els.inputsContainer;if(!container)return;const card=document.createElement('div');card.className='sensor-card';card.id=`input-${id}`;card.innerHTML=`<div class="card-header" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;"><h4 style="margin:0;font-size:1rem;">Input ${id+1}</h4><button type="button" class="remove-btn" onclick="document.getElementById('input-${id}').remove();markUnsaved()">Remove</button></div><div class="form-grid"><label class="field"><span>Input Name</span><input type="text" class="input-name" value="Clear Button" onchange="markUnsaved()"></label><label class="field"><span>Pin Number</span><input type="number" class="input-pin" value="0" min="0" max="99" onchange="markUnsaved()"></label><label class="field"><span>Input Mode<span class="tooltip-icon" tabindex="0" data-tooltip="Active LOW: Button connects to ground, uses internal pullup resistor (most common). Active HIGH: Button connects to VCC, requires external pull-down resistor.">?</span></span><select class="input-mode" onchange="markUnsaved()">${inputModes.map(m=>`<option value="${m.value}">${m.label}</option>`).join('')}</select></label><label class="field"><span>Action<span class="tooltip-icon" tabindex="0" data-tooltip="Clear All Relays: Turns off all active relay outputs and resets alarm states when button is pressed.">?</span></span><select class="input-action" onchange="markUnsaved()">${inputActions.map(a=>`<option value="${a.value}">${a.label}</option>`).join('')}</select></label></div>`;container.appendChild(card);inputIdCounter++;markUnsaved();};
window.toggleAlarmSection=funct)HTML" R"HTML(ion(id){const card=document.getElementById(`sensor-${id}`);const alarmSection=card.querySelector('.alarm-section');const addAlarmBtn=card.querySelector('.add-alarm-btn');const addRelayBtn=card.querySelector('.add-relay-btn');const addSmsBtn=card.querySelector('.add-sms-btn');alarmSection.classList.add('visible');addAlarmBtn.classList.add('hidden');addRelayBtn.classList.remove('hidden');addSmsBtn.classList.remove('hidden');};
window.removeAlarmSection=funct)HTML" R"HTML(ion(id){const card=document.getElementById(`sensor-${id}`);const alarmSection=card.querySelector('.alarm-section');const addAlarmBtn=card.querySelector('.add-alarm-btn');const addRelayBtn=card.querySelector('.add-relay-btn');const addSmsBtn=card.querySelector('.add-sms-btn');const relaySection=card.querySelector('.relay-section');const smsSection=card.querySelector('.sms-section');alarmSection.classList.remove('visible');addAlarmBtn.classList.remove('hidden');addRelayBtn.classList.add('hidden');addSmsBtn.classList.add('hidden');if(relaySection)relaySection.remove();if(smsSection)smsSection.remove();};
window.toggleRelaySection=funct)HTML" R"HTML(ion(id){const card=document.getElementById(`sensor-${id}`);let relaySection=card.querySelector('.relay-section');if(!relaySection){relaySection=document.createElement('div');relaySection.className='collapsible-section relay-section visible';relaySection.innerHTML=`<h4 style="margin:16px 0 8px;font-size:0.95rem;">Relay Control<button type="button" class="remove-btn" onclick="document.getElementById('sensor-${id}').querySelector('.relay-section').remove();document.getElementById('sensor-${id}').querySelector('.add-relay-btn').classList.remove('hidden')" style="float:right;">Remove Relay</button></h4><div class="form-grid"><label class="field"><span>Target Client UID<span class="tooltip-icon" tabindex="0" data-tooltip="Enter the UID of the client device with relay outputs to control. Can be this same client (local control) or a different client (remote control). Format: dev:IMEI">?</span></span><input type="text" class="relay-target" placeholder="dev:864475044012345"></label><label class="field"><span>Trigger On</span><select class="relay-trigger"><option value="any">Any Alarm</option><option value="high">High Alarm Only</option><option value="low">Low Alarm Only</option></select></label><label class="field"><span>Relay Mode<span class="tooltip-icon" tabindex="0" data-tooltip="Momentary: Pulse relay for short duration then turn off. Until Clear: Keep relay ON until alarm condition clears. Manual Reset: Stay ON until manually cleared via button or dashboard.">?</span></span><select class="relay-mode"><option value="momentary">Momentary</option><option value="until_clear">Until Alarm Clears</option><option value="manual_reset">Manual Reset</option></select></label><div class="field"><span>Relay Outputs</span><div style="display:flex;gap:12px;padding:8px 0;"><label><input type="checkbox" class="relay-1" value="1"> R1</label><label><input type="checkbox" class="relay-2" value="2"> R2</label><label><input type="checkbox" class="relay-3" value="4"> R3</label><label><input type="checkbox" class="relay-4" value="8"> R4</label></div></div></div>`;card.querySelector('.add-sms-btn').before(relaySection);}relaySection.classList.add('visible');card.querySelector('.add-relay-btn').classList.add('hidden');markUnsaved();};
window.toggleSmsSection=funct)HTML" R"HTML(ion(id){const card=document.getElementById(`sensor-${id}`);let smsSection=card.querySelector('.sms-section');if(!smsSection){smsSection=document.createElement('div');smsSection.className='collapsible-section sms-section visible';smsSection.innerHTML=`<h4 style="margin:16px 0 8px;font-size:0.95rem;">SMS Alert<button type="button" class="remove-btn" onclick="document.getElementById('sensor-${id}').querySelector('.sms-section').remove();document.getElementById('sensor-${id}').querySelector('.add-sms-btn').classList.remove('hidden')" style="float:right;">Remove SMS</button></h4><div class="form-grid"><label class="field" style="grid-column:1 / -1;"><span>Phone Numbers</span><input type="text" class="sms-phones" placeholder="+15551234567,+15559876543"></label><label class="field"><span>Trigger On</span><select class="sms-trigger"><option value="any">Any Alarm</option><option value="high">High Alarm Only</option><option value="low">Low Alarm Only</option></select></label><label class="field" style="grid-column:span 2;"><span>Custom Message</span><input type="text" class="sms-message" placeholder="Tank alarm triggered"></label></div>`;card.appendChild(smsSection);}smsSection.classList.add('visible');card.querySelector('.add-sms-btn').classList.add('hidden');};
window.updateMonitorFields=funct)HTML" R"HTML(ion(id){const card=document.getElementById(`sensor-${id}`);const type=card.querySelector('.monitor-type').value;const numField=card.querySelector('.tank-num-field');const numFieldLabel=numField.querySelector('span');const nameLabel=card.querySelector('.name-label');const heightLabel=card.querySelector('.height-label');const contentsField=card.querySelector('.contents-field');const levelChangeField=card.querySelector('.level-change-field');if(type==='gas'){numField.style.display='none';nameLabel.textContent='System Name';heightLabel.textContent='Max Pressure';contentsField.style.display='flex';levelChangeField.style.display='none';}else if(type==='rpm'){numField.style.display='flex';numFieldLabel.textContent='Engine Number';nameLabel.textContent='Engine Name';heightLabel.textContent='Max RPM';contentsField.style.display='none';levelChangeField.style.display='none';}else{numField.style.display='flex';numFieldLabel.textContent='Tank Number';nameLabel.textContent='Name';heightLabel.textContent='Height(in)';contentsField.style.display='flex';const sensorType=parseInt(card.querySelector('.sensor-type').value);levelChangeField.style.display=(sensorType!==0)?'flex':'none';}updateSensorTypeFields(id);};
window.updatePinOptions=funct)HTML" R"HTML(ion(id){const card=document.getElementById(`sensor-${id}`);const typeSelect=card.querySelector('.sensor-type');const pinSelect=card.querySelector('.sensor-pin');const type=parseInt(typeSelect.value);pinSelect.innerHTML='';const options=(type===2)?expansionChannels:optaPins;options.forEach(opt=>{const option=document.createElement('option');option.value=opt.value;option.textContent=opt.label;pinSelect.appendChild(option);});updateSensorTypeFields(id);};
window.updateSensorTypeFields=funct)HTML" R"HTML(ion(id){const card=document.getElementById(`sensor-${id}`);const type=parseInt(card.querySelector('.sensor-type').value);const monitorType=card.querySelector('.monitor-type').value;const heightField=card.querySelector('.height-field');const switchModeField=card.querySelector('.switch-mode-field');const pulsesPerRevField=card.querySelector('.pulses-per-rev-field');const currentLoopTypeField=card.querySelector('.current-loop-type-field');const levelChangeField=card.querySelector('.level-change-field');const alarmThresholdsGrid=card.querySelector('.alarm-thresholds-grid');const digitalAlarmGrid=card.querySelector('.digital-alarm-grid');if(type===0){heightField.style.display='none';switchModeField.style.display='flex';pulsesPerRevField.style.display='none';currentLoopTypeField.style.display='none';levelChangeField.style.display='none';alarmThresholdsGrid.style.display='none';digitalAlarmGrid.style.display='grid';}else if(type===2){heightField.style.display='flex';switchModeField.style.display='none';pulsesPerRevField.style.display='none';currentLoopTypeField.style.display='flex';levelChangeField.style.display=(monitorType==='tank')?'flex':'none';alarmThresholdsGrid.style.display='grid';digitalAlarmGrid.style.display='none';}else if(type===3){heightField.style.display='flex';switchModeField.style.display='none';pulsesPerRevField.style.display='flex';currentLoopTypeField.style.display='none';levelChangeField.style.display='none';alarmThresholdsGrid.style.display='grid';digitalAlarmGrid.style.display='none';}else{heightField.style.display='flex';switchModeField.style.display='none';pulsesPerRevField.style.display='none';currentLoopTypeField.style.display='none';levelChangeField.style.display=(monitorType==='tank')?'flex':'none';alarmThresholdsGrid.style.display='grid';digitalAlarmGrid.style.display='none';}};
window.updateCurrentLoopFields=funct)HTML" R"HTML(ion(id){};
funct)HTML" R"HTML(ion collectConfig(){const tanks=[];const sensorCards=els.sensorsContainer.querySelectorAll('.sensor-card');if(!sensorCards.length){throw new Error('Add at least one sensor before sending.');}sensorCards.forEach((card,index)=>{const monitorType=card.querySelector('.monitor-type').value;const type=parseInt(card.querySelector('.sensor-type').value);const pin=parseInt(card.querySelector('.sensor-pin').value);let tankNum=parseInt(card.querySelector('.tank-num').value)||(index+1);let name=card.querySelector('.tank-name').value;const contents=card.querySelector('.tank-contents')?.value||'';if(monitorType==='gas'){if(!name)name=`Gas System ${index+1}`;}else if(monitorType==='rpm'){if(!name)name=`Engine ${tankNum}`;}else{if(!name)name=`Tank ${index+1}`;}const sensor=sensorKeyFromValue(type);const pulsesPerRev=Math.max(1,Math.min(255,parseInt(card.querySelector('.pulses-per-rev').value)||1));const switchMode=card.querySelector('.switch-mode').value;const alarmSectionVisible=card.querySelector('.alarm-section').classList.contains('visible');const highAlarmEnabled=card.querySelector('.high-alarm-enabled').checked;const lowAlarmEnabled=card.querySelector('.low-alarm-enabled').checked;const highAlarmValue=card.querySelector('.high-alarm').value;const lowAlarmValue=card.querySelector('.low-alarm').value;const tank={id:String.fromCharCode(65+index),name:name,contents:contents,number:tankNum,sensor:sensor,primaryPin:sensor==='current'?0:pin,secondaryPin:-1,loopChannel:sensor==='current'?pin:-1,rpmPin:sensor==='rpm'?pin:-1,maxValue:sensor==='digital'?1:(parseFloat(card.querySelector('.tank-height').value)||120),levelChangeThreshold:(monitorType==='tank'&&sensor!=='digital')?(parseFloat(card.querySelector('.level-change-threshold').value)||0):0,hysteresis:sensor==='digital'?0:2.0,daily:true,upload:true};if(sensor==='digital'){tank.digitalSwitchMode=switchMode;}if(sensor==='rpm'){tank.pulsesPerRev=pulsesPerRev;}if(alarmSectionVisible){if(sensor==='digital'){const digitalTriggerState=card.querySelector('.digital-trigger-state').value;tank.digitalTrigger=digitalTriggerState;if(digitalTriggerState==='activated'){tank.highAlarm=1;}else{tank.lowAlarm=0;}tank.alarmSms=true;}else if(highAlarmEnabled||lowAlarmEnabled){if(highAlarmEnabled&&highAlarmValue!==''){const highAlarmFloat=parseFloat(highAlarmValue);if(!isNaN(highAlarmFloat)){tank.highAlarm=highAlarmFloat;}}if(lowAlarmEnabled&&lowAlarmValue!==''){const lowAlarmFloat=parseFloat(lowAlarmValue);if(!isNaN(lowAlarmFloat)){tank.lowAlarm=lowAlarmFloat;}}tank.alarmSms=true;}else{tank.alarmSms=false;}}else{tank.alarmSms=false;}const relaySection=card.querySelector('.relay-section');if(relaySection&&relaySection.classList.contains('visible')){let relayMask=0;['relay-1','relay-2','relay-3','relay-4'].forEach(cls=>{const checkbox=card.querySelector('.'+cls);if(checkbox.checked)relayMask|=parseInt(checkbox.value);});const relayTarget=card.querySelector('.relay-target').value.trim();const relayTrigger=card.querySelector('.relay-trigger').value;const relayMode=card.querySelector('.relay-mode').value;if(relayTarget){tank.relayTargetClient=relayTarget;tank.relayMask=relayMask;tank.relayTrigger=relayTrigger;tank.relayMode=relayMode;}}const smsSection=card.querySelector('.sms-section');if(smsSection&&smsSection.classList.contains('visible')){const smsPhones=card.querySelector('.sms-phones').value.trim();const smsTrigger=card.querySelector('.sms-trigger').value;const smsMessage=card.querySelector('.sms-message').value.trim();if(smsPhones){const phoneArray=smsPhones.split(',').map(p=>p.trim()).filter(p=>p.length>0);if(phoneArray.length>0){tank.smsAlert={phones:phoneArray,trigger:smsTrigger,message:smsMessage||'Tank alarm triggered'};}}}tanks.push(tank);});const powerSource=els.powerSource?els.powerSource.value:'grid';const config={site:els.site.value.trim(),deviceLabel:els.deviceLabel.value.trim(),serverFleet:els.route.value.trim()||'tankalarm-server',sampleSeconds:parseInt(els.sampleSeconds.value,10)||1800,reportHour:parseInt(els.reportHour.value,10)||5,reportMinute:parseInt(els.reportMinute.value,10)||0,dailyEmail:els.dailyEmail.value.trim(),powerSource:powerSource,solarPowered:(powerSource==='solar'||powerSource==='solar_mppt'||powerSource==='solar_modbus_mppt'),mpptEnabled:(powerSource==='solar_mppt'||powerSource==='solar_modbus_mppt'),solarCharger:{enabled:(powerSource==='solar_modbus_mppt')},tanks:tanks};const inputCards=els.inputsContainer.querySelectorAll('.sensor-card');let clearButtonConfigured=false;inputCards.forEach((card)=>{const inputName=card.querySelector('.input-name').value.trim()||'Input';const inputPin=parseInt(card.querySelector('.input-pin').value,10);const inputMode=card.querySelector('.input-mode').value;const inputAction=card.querySelector('.input-action').value;if(inputAction==='clear_relays'&&!clearButtonConfigured){config.clearButtonPin=isNaN(inputPin)?-1:inputPin;config.clearButtonActiveHigh=(inputMode==='active_high');clearButtonConfigured=true;}});if(!clearButtonConfigured){config.clearButtonPin=-1;config.clearButtonActiveHigh=false;}return config;}
funct)HTML" R"HTML(ion collectServerSettings(){return{smsPrimary:els.smsPrimary.value.trim(),smsSecondary:els.smsSecondary.value.trim()};}
funct)HTML" R"HTML(ion validateConfigBeforeSend(config){const errors=[];if(!config.site||!config.site.trim()){errors.push('Site Name is required');}if(!config.deviceLabel||!config.deviceLabel.trim()){errors.push('Device Label is required');}if(config.sensors&&config.sensors.length>0){const usedPins=new Set();config.sensors.forEach((sensor,idx)=>{if(sensor.pin){if(usedPins.has(sensor.pin)){errors.push(`Sensor ${idx+1}: Pin ${sensor.pin} is already in use by another sensor`);}usedPins.add(sensor.pin);}if(sensor.monitor==='tank'&&(!sensor.maxValue||sensor.maxValue<=0)){errors.push(`Sensor ${idx+1}: Tank sensors must have a valid Max Height/Capacity`);}if(sensor.sensorType==='current_loop'){if(!sensor.currentLoopMin||sensor.currentLoopMin<4||sensor.currentLoopMin>20){errors.push(`Sensor ${idx+1}: Current loop minimum must be 4-20 mA`);}if(!sensor.currentLoopMax||sensor.currentLoopMax<4||sensor.currentLoopMax>20||sensor.currentLoopMax<=sensor.currentLoopMin){errors.push(`Sensor ${idx+1}: Current loop maximum must be valid and greater than minimum`);}}if(sensor.alarms&&sensor.alarms.length>0){sensor.alarms.forEach((alarm,aIdx)=>{if(!alarm.type||!alarm.threshold){errors.push(`Sensor ${idx+1}, Alarm ${aIdx+1}: Must specify type and threshold`);}});}if(sensor.relays&&sensor.relays.length>0){sensor.relays.forEach((relay,rIdx)=>{if(!relay.pin||!relay.targetClientUid||!relay.targetClientUid.trim()){errors.push(`Sensor ${idx+1}, Relay ${rIdx+1}: Must specify pin and target client UID`);}});}if(sensor.smsAlerts&&sensor.smsAlerts.length>0){sensor.smsAlerts.forEach((sms,sIdx)=>{if(!sms.phone||!sms.phone.trim()||sms.phone.length<10){errors.push(`Sensor ${idx+1}, SMS Alert ${sIdx+1}: Must specify valid phone number (10+ digits)`);}});}});}if(config.inputs&&config.inputs.length>0){config.inputs.forEach((input,idx)=>{if(input.pin&&usedPins.has(input.pin)){errors.push(`Input ${idx+1}: Pin ${input.pin} is already in use by a sensor`);}});}return errors;}
funct)HTML" R"HTML(ion downloadConfigAsJson(){let config;try{config=collectConfig();}catch(err){showToast(err.message,true);return;}const validationErrors=validateConfigBeforeSend(config);if(validationErrors.length>0){showToast(`Validation errors: ${validationErrors.join('; ')}`,true);return;}const clientUid=state.selected||'unknown-client';const timestamp=new Date().toISOString().replace(/[:.]/g,'-').slice(0,19);const filename=`TankAlarm_${config.site||'config'}_${clientUid.slice(-6)}_${timestamp}.json`;const jsonStr=JSON.stringify(config,null,2);const blob=new Blob([jsonStr],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=filename;document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);showToast('Configuration downloaded as '+filename);}
async funct)HTML" R"HTML(ion submitConfig(event){event.preventDefault();const uid = state.selected;if(!uid){showToast('Select a client first.',true);return;}let config;try{config = collectConfig();}catch(err){showToast(err.message,true);return;}const validationErrors=validateConfigBeforeSend(config);if(validationErrors.length>0){showToast(`Cannot send: ${validationErrors.join('; ')}`,true);return;}if(state.originalConfig){const diff=compareConfigs(state.originalConfig,config);const comparisonHtml=displayConfigComparison(diff);if(diff.added.length||diff.modified.length||diff.removed.length){const proceed=confirm(`Review configuration changes before sending:\n\nModified: ${diff.modified.length} fields\nAdded: ${diff.added.length} fields\nRemoved: ${diff.removed.length} fields\n\nContinue with send?`);if(!proceed)return;}}if(els.configStatus){els.configStatus.style.display='block';els.configStatus.textContent='Validating configuration...';els.configStatus.className='config-pending';}if(pinState.configured && !pinState.value){showPinModal();showToast('Enter the admin PIN to send configurations.',true);if(els.configStatus)els.configStatus.style.display='none';return;}const payload ={client:uid,config,server:collectServerSettings()};if(pinState.value){payload.pin = pinState.value;}try{if(els.configStatus){els.configStatus.textContent='Sending configuration to server...';els.configStatus.className='config-pending';}const res = await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!res.ok){if(res.status === 403){invalidatePin();showPinModal();throw new Error('PIN required or invalid.');}const text = await res.text();throw new Error(text || 'Server rejected configuration');}logConfigChange(uid,state.originalConfig?compareConfigs(state.originalConfig,config):{added:[],modified:[],removed:[]},Date.now());state.originalConfig=JSON.parse(JSON.stringify(config));clearUnsaved();if(els.configStatus){els.configStatus.textContent='✓ Configuration queued for delivery to client';els.configStatus.className='config-success';setTimeout(()=>{if(els.configStatus)els.configStatus.style.display='none';},5000);}showToast('Configuration queued for delivery');await refreshData(state.selected);}catch(err){if(els.configStatus){els.configStatus.textContent='✗ Failed: '+err.message;els.configStatus.className='config-error';setTimeout(()=>{if(els.configStatus)els.configStatus.style.display='none';},8000);}showToast(err.message || 'Failed to send config',true);}}
funct)HTML" R"HTML(ion normalizeApiData(data){var srv=data.srv||{};var result={server:{name:srv.n||'',smsPrimary:srv.sp||'',smsSecondary:srv.ss||'',productUid:srv.pu||'',pinConfigured:!!srv.pc,dailyEmail:srv.de||''},serverUid:data.si||'',nextDailyEmailEpoch:data.nde||0,clients:(data.cs||[]).map(function(c){return{client:c.c||'',site:c.s||'',label:c.n||'',tank:c.k||'',alarm:!!c.a,alarmType:c.at||'',lastUpdate:c.u||0,levelInches:c.l,vinVoltage:c.v,firmwareVersion:c.fv||'',tanks:(c.ts||[]).map(function(t){return{label:t.n||'',tank:t.k||'',levelInches:t.l,heightInches:t.h,alarm:!!t.a,alarmType:t.at||'',lastUpdate:t.u||0};})};})};result.configs=(data.cfgs||[]).map(function(cfg){return{client:cfg.c||'',site:cfg.s||'',configJson:cfg.cj||''};});return result;})HTML"
R"HTML(funct)HTML" R"HTML(ion applyServerData(data,preferredUid){state.data=normalizeApiData(data);const serverInfo=state.data.server;if(els.serverName)els.serverName.textContent=serverInfo.name||'Tank Alarm Server';syncServerSettings(serverInfo);if(els.serverUid)els.serverUid.textContent=state.data.serverUid||'--';if(els.nextEmail)els.nextEmail.textContent=formatEpoch(state.data.nextDailyEmailEpoch);if(preferredUid){state.selected=preferredUid;}renderTelemetry();renderNewSites();populateClientSelect();if(state.selected){loadConfigIntoForm(state.selected);}else if(els.clientSelect.options.length){loadConfigIntoForm(els.clientSelect.value);}updatePinLock();}
...
funct)HTML" R"HTML(ion markUnsaved(){state.hasUnsavedChanges=true;if(els.sendConfigBtn){els.sendConfigBtn.textContent='Send Configuration *';els.sendConfigBtn.style.fontWeight='600';}}
funct)HTML" R"HTML(ion clearUnsaved(){state.hasUnsavedChanges=false;if(els.sendConfigBtn){els.sendConfigBtn.textContent='Send Configuration';els.sendConfigBtn.style.fontWeight='400';}}
window.markUnsaved=markUnsaved;
window.addEventListener('beforeunload',e=>{if(state.hasUnsavedChanges){e.preventDefault();e.returnValue='You have unsaved configuration changes. Are you sure you want to leave?';return e.returnValue;}});
funct)HTML" R"HTML(ion importConfigAsJson(){if(els.importFileInput){els.importFileInput.click();}}
async funct)HTML" R"HTML(ion handleFileImport(event){const file=event.target.files[0];if(!file)return;try{const text=await file.text();const config=JSON.parse(text);const validationErrors=validateConfigBeforeSend(config);if(validationErrors.length>0){showToast(`Invalid JSON: ${validationErrors.join('; ')}`,true);return;}if(config.site)els.site.value=config.site||'';if(config.deviceLabel)els.deviceLabel.value=config.deviceLabel||'';if(config.serverFleet)els.route.value=config.serverFleet||'';if(config.sampleSeconds!==undefined)els.sampleSeconds.value=config.sampleSeconds;if(config.levelChangeThreshold!==undefined)els.levelChangeThreshold.value=config.levelChangeThreshold;if(config.reportHour!==undefined)els.reportHour.value=config.reportHour;if(config.reportMinute!==undefined)els.reportMinute.value=config.reportMinute;if(config.dailyEmail)els.dailyEmail.value=config.dailyEmail||'';if(config.powerSource&&els.powerSource)els.powerSource.value=config.powerSource||'grid';sensorCount=0;inputIdCounter=0;if(els.sensorsContainer)els.sensorsContainer.innerHTML='';if(els.inputsContainer)els.inputsContainer.innerHTML='';if(config.tanks&&config.tanks.length){config.tanks.forEach(tank=>addSensorFromConfig(tank));}if(config.clearButtonPin&&config.clearButtonPin>=0){addInputFromConfig({name:'Clear Button',pin:config.clearButtonPin,mode:config.clearButtonActiveHigh?'active_high':'active_low',action:'clear_relays'});}markUnsaved();showToast(`Configuration loaded from ${file.name}`);}catch(err){showToast(`Failed to import: ${err.message}`,true);}if(els.importFileInput)els.importFileInput.value='';}
funct)HTML" R"HTML(ion compareConfigs(oldConfig,newConfig){const diff={added:[],modified:[],removed:[]};const keys=new Set([...Object.keys(oldConfig||{}),...Object.keys(newConfig||{})]);keys.forEach(key=>{const oldVal=oldConfig?oldConfig[key]:undefined;const newVal=newConfig?newConfig[key]:undefined;if(oldVal===undefined&&newVal!==undefined){diff.added.push({field:key,value:newVal});}else if(oldVal!==undefined&&newVal===undefined){diff.removed.push({field:key,value:oldVal});}else if(JSON.stringify(oldVal)!==JSON.stringify(newVal)){diff.modified.push({field:key,oldValue:oldVal,newValue:newVal});}});return diff;}
funct)HTML" R"HTML(ion displayConfigComparison(diff){if(!diff.added.length&&!diff.modified.length&&!diff.removed.length){return 'No changes detected';}let html='<div style="max-height:300px;overflow-y:auto;padding:8px;background:var(--chip);border:1px solid var(--card-border);border-radius:var(--radius);margin:8px 0;">';if(diff.added.length){html+='<strong style="color:#10b981;">Added:</strong><ul>';diff.added.forEach(item=>html+=`<li>+${item.field}</li>`);html+='</ul>';}if(diff.modified.length){html+='<strong style="color:#f59e0b;">Modified:</strong><ul>';diff.modified.forEach(item=>html+=`<li>•${item.field}</li>`);html+='</ul>';}if(diff.removed.length){html+='<strong style="color:#dc2626;">Removed:</strong><ul>';diff.removed.forEach(item=>html+=`<li>-${item.field}</li>`);html+='</ul>';}html+='</div>';return html;}
funct)HTML" R"HTML(ion logConfigChange(clientUid,changes,timestamp){const entry={timestamp:timestamp||Date.now(),clientUid:clientUid||state.selected||'unknown',changes:changes,user:'admin'};state.configHistory.push(entry);if(state.configHistory.length>50){state.configHistory.shift();}try{localStorage.setItem('tankalarm_config_history',JSON.stringify(state.configHistory));}catch(err){console.warn('Failed to save config history',err);}}
funct)HTML" R"HTML(ion loadConfigHistory(){try{const stored=localStorage.getItem('tankalarm_config_history');if(stored){state.configHistory=JSON.parse(stored)||[];}}catch(err){console.warn('Failed to load config history',err);state.configHistory=[];}}
pinEls.form.addEventListener('submit',handlePinSubmit);pinEls.cancel.addEventListener('click',()=>{hidePinModal();});if(els.downloadConfigBtn)els.downloadConfigBtn.addEventListener('click',downloadConfigAsJson);if(els.importConfigBtn)els.importConfigBtn.addEventListener('click',importConfigAsJson);if(els.importFileInput)els.importFileInput.addEventListener('change',handleFileImport);if(els.addSensor)els.addSensor.addEventListener('click',()=>addSensor());if(els.addInput)els.addInput.addEventListener('click',()=>addInput());if(els.clientSelect)els.clientSelect.addEventListener('change',event =>{if(state.hasUnsavedChanges){if(!confirm('You have unsaved changes. Switch clients anyway?')){els.clientSelect.value=state.selected;return;}}clearUnsaved();loadConfigIntoForm(event.target.value);fetchLocationInfo(event.target.value);});if(els.form)els.form.addEventListener('submit',submitConfig);if(els.refreshSelectedBtn)els.refreshSelectedBtn.addEventListener('click',()=>{const target = state.selected ||(els.clientSelect.value || '');if(!target){showToast('Select a client first.',true);return;}triggerManualRefresh(target);});if(els.refreshAllBtn)els.refreshAllBtn.addEventListener('click',()=>{triggerManualRefresh(null);});if(els.requestLocationBtn)els.requestLocationBtn.addEventListener('click',()=>{const target = state.selected ||(els.clientSelect.value || '');requestLocation(target);});loadConfigHistory();refreshData().then(()=>{if(state.selected)fetchLocationInfo(state.selected);});})();
</script></body></html>)HTML";
...
static void serveCss(EthernetClient &client) {
  ...
  client.println(F("Cache-Control: public, max-age=31536000"));
  client.println();
