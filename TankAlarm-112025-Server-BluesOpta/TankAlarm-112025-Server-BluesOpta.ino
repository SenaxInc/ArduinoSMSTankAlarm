...
window.derivePowerSource=function(){
  /* Synthesize legacy powerSource string from the 4 new fields so older clients
     still parse a recognizable value. */
  ...
  const noBat=(bt==='none');
  if(ps==='grid'){
    if(noBat)return 'grid';
    return vin?'grid_battery_vin':'grid_battery';
  }
  /* solar family */
  if(noBat)return vin?'solar_nobat_vin':'solar_nobat';
  if(ps==='solar_modbus_mppt')return 'solar_modbus_mppt';
  if(ps==='solar_mppt')return 'solar_mppt';
  return vin?'solar_vin':'solar';
};
...
/* New 4-field power model — derive from batteryConfig if present, else parse legacy powerSource */
const legacyPs=c.powerSource||'grid';
let psSel=c.powerSupply||'';
if(!psSel){if(legacyPs==='solar_modbus_mppt')psSel='solar_modbus_mppt';else if(legacyPs==='solar_mppt')psSel='solar_mppt';else if(legacyPs.startsWith('solar'))psSel='solar';else psSel='grid';}
...
else if(c.batteryConfig&&typeof c.batteryConfig.batteryType==='number'){const revMap={0:'agm',1:'lifepo4',2:'lipo',3:'custom',4:'none',5:'agm',6:'flooded',7:'gel',8:'sla',9:'lifepo4',10:'li_ion'};btSel=revMap[c.batteryConfig.batteryType]||'agm';bvSel=c.batteryConfig.nominalVoltage||12;if(!c.batteryConfig.enabled)btSel='none';}
else{/* derive from legacy powerSource */if(legacyPs.includes('nobat')||legacyPs==='grid')btSel='none';else btSel='agm';bvSel=12;}
