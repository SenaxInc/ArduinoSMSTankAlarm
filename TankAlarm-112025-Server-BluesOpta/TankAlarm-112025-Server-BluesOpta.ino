...
const state={pin:null,pinConfigured:false,paused:false,clients:[]};
...
const pause_state={paused:false,pin:null,pinConfigured:false};if(pause_els.btn)pause_els.btn.addEventListener('click',togglePauseFlow);if(pause_els.btn)pause_els.btn.addEventListener('mouseenter',()=>{if(pause_state.paused)pause_els.btn.textContent='Resume';});if(pause_els.btn)pause_els.btn.addEventListener('mouseleave',()=>{renderPauseBtn();});await fetch('/api/clients?summary=1').then(r=>r.json()).then(d=>{if(d&&d.srv){pause_state.paused=!!d.srv.ps;pause_state.pinConfigured=!!d.srv.pc;renderPauseBtn();}}).catch(e=>console.error('Failed to load pause state',e));
...
R"HTML(async function cancelPendingConfig(clientUid){if(!confirm('Cancel pending config dispatch for '+clientUid+'? This will stop auto-retries and purge unsent notes from the Notecard outbox.'))return;const _pin=prompt('Enter admin PIN');if(!_pin)return;try{const res=await fetch('/api/config/cancel',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pin:_pin,client:clientUid})});const text=await res.text();if(res.ok){showToast(text||'Config dispatch cancelled');}else{showToast(text||'Cancel failed',true);}await loadLog();}catch(err){showToast('Error: '+err.message,true);}}async function cancelAllPendingConfigs(){if(!confirm('Cancel ALL pending config dispatches? This will stop all auto-retries.'))return;const _pin=prompt('Enter admin PIN');if(!_pin)return;try{const res=await fetch('/api/config/cancel',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pin:_pin})});const text=await res.text();if(res.ok){showToast(text||'All pending configs cancelled');}else{showToast(text||'Cancel failed',true);}await loadLog();}catch(err){showToast('Error: '+err.message,true);}}loadLog();setInterval(loadLog,15000);})();</script></body></html>)HTML";
...
const state={clients:[],sites:{},refreshing:false,timer:null,uiRefreshS:DEFAULT_REFRESH_S,paused:false,pin:null,pinConfigured:false,expandedDot:null,sparkData:{}};
...
async function refreshTank(clientUid){if(state.refreshing)return;if(state.pinConfigured&&!state.pin){const p=prompt('Enter admin PIN to refresh');if(!p)return;state.pin=p.trim();}state.refreshing=true;renderSites();try{const res=await fetch('/api/refresh',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({client:clientUid,pin:state.pin||''})});if(!res.ok){if(res.status===403){state.pin=null;throw new Error('PIN required or invalid');}throw new Error(await res.text()||'Refresh failed');}applyServerData(await res.json());showToast('Refreshed');}catch(err){showToast(err.message||'Refresh failed',true);}finally{state.refreshing=false;renderSites();}}
