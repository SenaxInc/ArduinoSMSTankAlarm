...
async function submitConfig(e){e.preventDefault();requestPin(async(pin)=>{try{const cfg=collectConfig();const res=await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pin:pin,config:cfg})});if(res.ok){showToast('Configuration saved to device');}else{const t=await res.text();showToast('Error: '+t,true);}}catch(err){showToast('Error: '+err.message,true);}});}if(els.form)els.form.addEventListener('submit',submitConfig);
...
window.fetchClientConfig=async(uid)=>{closeSelectClientModal();try{const r=await fetch('/api/client?uid='+encodeURIComponent(uid));if(!r.ok)throw new Error('Failed');const c=await r.json();if(c.config)loadConfig(c.config);else showToast('No config found for client',true);}catch(e){showToast('Error loading config',true);}};
...
async function basicInit(){try{const r=await fetch('/api/clients?summary=1');const d=await r.json();if(d&&d.srv&&d.srv.pu)els.productUid.value=d.srv.pu;}catch(e){}addSensor();}basicInit();
})();
