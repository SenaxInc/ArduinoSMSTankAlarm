...
static void handleTelemetry(JsonDocument &doc, double epoch) {
  ...
  if (doc.containsKey("h")) {
    rec->heightInches = doc["h"].as<float>();
  } else if (doc.containsKey("heightInches")) {
...
static void handleAlarm(JsonDocument &doc, double epoch) {
  ...
  float inches = doc["l"] | doc["levelInches"].as<float>();
