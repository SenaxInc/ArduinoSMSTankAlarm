...
(()=>{const token=localStorage.getItem('tankalarm_token');if(!token){window.location.href='/login?redirect='+encodeURIComponent(window.location.pathname);return;}
setTimeout(()=>{const o=document.getElementById('loading-overlay');if(o)o.style.display='none';},8000);window.addEventListener('error',()=>{const o=document.getElementById('loading-overlay');if(o)o.style.display='none';});
...
static void respondHtml(EthernetClient &client, const String &body) {
  ...
  if (output.indexOf("loading-overlay") < 0) {
    ...
    const char *hideScript = "<script>window.addEventListener('load',()=>{const ov=document.getElementById('loading-overlay');if(ov){ov.style.display='none';ov.classList.add('hidden');}});</script>";
