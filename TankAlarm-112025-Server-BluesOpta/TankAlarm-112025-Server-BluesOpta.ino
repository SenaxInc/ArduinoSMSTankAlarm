...
static void handleServerSettingsPost(EthernetClient &client, const String &body) {
  ...
  if (doc.containsKey("server")) {
    ...
    if (serverObj.containsKey("reportHour")) {
      globalServerConfig.reportHour = serverObj["reportHour"] | 5;
    }
    if (serverObj.containsKey("reportMinute")) {
      globalServerConfig.reportMinute = serverObj["reportMinute"] | 0;
    }
    if (serverObj.containsKey("dailyEmail")) {
