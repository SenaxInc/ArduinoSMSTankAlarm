...
// Also checks that an active session exists (prevents stale tabs from making changes after logout).
...
<script>
(async () => {
const token=localStorage.getItem('tankalarm_token');const _s=localStorage.getItem('tankalarm_session');if(!token||!_s){window.location.href='/login?redirect='+encodeURIComponent(window.location.pathname);return;}async function _ckSess(){const sid=localStorage.getItem('tankalarm_session');if(!sid){window.location.href='/login?reason=expired';return;}try{const r=await fetch('/api/session/check?token='+sid);const d=await r.json();if(!d.valid){localStorage.removeItem('tankalarm_token');localStorage.removeItem('tankalarm_session');window.location.href='/login?reason=expired';}}catch(e){}}document.addEventListener('visibilitychange',()=>{if(!document.hidden)_ckSess();});setInterval(_ckSess,30000);
...
(()=>{setTimeout(()=>{const o=document.getElementById('loading-overlay');if(o)o.style.display='none';},8000);window.addEventListener('error',()=>{const o=document.getElementById('loading-overlay');if(o)o.style.display='none';});const token=localStorage.getItem('tankalarm_token');const _s=localStorage.getItem('tankalarm_session');if(!token||!_s){window.location.href='/login?redirect='+encodeURIComponent(window.location.pathname);return;}async function _ckSess(){const sid=localStorage.getItem('tankalarm_session');if(!sid){window.location.href='/login?reason=expired';return;}try{const r=await fetch('/api/session/check?token='+sid);const d=await r.json();if(!d.valid){localStorage.removeItem('tankalarm_token');localStorage.removeItem('tankalarm_session');window.location.href='/login?reason=expired';}}catch(e){}}document.addEventListener('visibilitychange',()=>{if(!document.hidden)_ckSess();});setInterval(_ckSess,30000);
...
function toggleDotInfo(section,cl,siteName){const slot=section.querySelector('.cdot-info-slot');if(state.expandedDot===cl.uid){slot.innerHTML='';state.expandedDot=null;return;}state.expandedDot=cl.uid;const color=clientStatusColor(cl);const statusText=cl.alarm?'ALARM':isStale(cl.lastUpdate)?'Stale / No Data':'Online';const tankCount=cl.tanks.length;slot.innerHTML=`<div class="cdot-info open"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;"><div><strong>${escapeHtml(cl.uid)}</strong><span class="status-pill ${color==='red'?'alarm':color==='yellow'?'stale':'ok'}" style="margin-left:8px;">${statusText}</span></div><div style="display:flex;gap:6px;"><a class="pill secondary" style="font-size:0.75rem;padding:2px 10px;" href="/config-generator?uid=${encodeURIComponent(cl.uid)}">Edit Config</a><a class="pill secondary" style="font-size:0.75rem;padding:2px 10px;" href="/site-config?site=${encodeURIComponent(siteName)}">Site Config</a><button class="pill secondary" style="font-size:0.75rem;padding:2px 10px;color:var(--danger);border-color:var(--danger);" onclick="deleteClient('${escapeHtml(cl.uid)}')">Remove Client</button></div></div><div style="margin-top:6px;color:var(--muted);font-size:0.85rem;">${tankCount} sensor${tankCount!==1?'s':''} &middot; Last update: ${formatEpoch(cl.lastUpdate)} &middot; VIN: ${formatVoltage(cl.vinVoltage)}</div></div>`;}
...
async function deleteClient(clientUid){if(!confirm('Remove client '+clientUid+' and all its tank data? This cannot be undone.'))return;if(state.pinConfigured&&!state.pin){const p=prompt('Enter admin PIN to remove client');if(!p)return;state.pin=p.trim();}try{const res=await fetch('/api/client',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({client:clientUid,pin:state.pin||''})});if(!res.ok){if(res.status===403){state.pin=null;throw new Error('PIN required or invalid');}throw new Error(await res.text()||'Delete failed');}showToast('Client removed: '+clientUid);state.expandedDot=null;refreshData();}catch(err){showToast(err.message||'Failed to remove client',true);}}
...
(function(){const token=localStorage.getItem('tankalarm_token');const _s=localStorage.getItem('tankalarm_session');if(!token||!_s){window.location.href='/login?redirect='+encodeURIComponent(window.location.pathname);return;}async function _ckSess(){const sid=localStorage.getItem('tankalarm_session');if(!sid){window.location.href='/login?reason=expired';return;}try{const r=await fetch('/api/session/check?token='+sid);const d=await r.json();if(!d.valid){localStorage.removeItem('tankalarm_token');localStorage.removeItem('tankalarm_session');window.location.href='/login?reason=expired';}}catch(e){}}document.addEventListener('visibilitychange',()=>{if(!document.hidden)_ckSess();});setInterval(_ckSess,30000);
...
static void handleSessionCheck(EthernetClient &client, const String &path) {
  // Extract token from query string: /api/session/check?token=...
  int idx = path.indexOf("token=");
  bool valid = false;
  if (idx >= 0) {
    String tok = path.substring(idx + 6);
    int amp = tok.indexOf('&');
    if (amp >= 0) tok = tok.substring(0, amp);
    valid = sessionTokenMatches(tok.c_str());
  }
  if (valid) {
...
static void handleWebRequests() {
  ...
  if (method == "GET" && path.startsWith("/login")) {
    ...
    handleSessionCheck(client, path);
