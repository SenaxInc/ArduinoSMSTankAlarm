...
funct)HTML" R"HTML(ion renderPauseBtn(){const btn=pause_els.btn;if(!btn)return;if(pause_state.paused){btn.classList.add('paused');btn.textContent='Paused';btn.title='Paused - hover to resume';}else{btn.classList.remove('paused');btn.textContent='Pause';btn.title='Pause data flow';}}
