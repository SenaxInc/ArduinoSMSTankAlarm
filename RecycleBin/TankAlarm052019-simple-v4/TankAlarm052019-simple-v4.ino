...
volatile int time_tick_sleep = 1; //start tick count at 1
volatile int time_tick_report = 1; //start tick count at 1
volatile int tick_socket = 1; //integer to count socket retries

const int sleep_minutes = 30;  //time between sensor checks
const int ticks_per_sleep = (sleep_minutes*60)/8;
const int report_hours = 6;    //time between mandatory status message send
const int ticks_per_report = ((report_hours*60*60)/8)-225;  //default 59 min of 8 second ticks 59*60/8  
const int fail_minutes = 10;
const int ticks_per_fail = (fail_minutes*60)/8;

int failSendD = 0;  //keep track of last transmission timeout
int failSendA = 0;
...
void loop() {
  ...
  if (time_tick_report > ticks_per_report && time_tick_sleep > ticks_per_sleep) {   //if number of ticks has reached 24 hours worth send text no matter what
  ...
  else if (time_tick_sleep > ticks_per_sleep) {  //if number of ticks has reach hour goal send text 
    //turn off watchdog timer while communicating
    wdt_disable();
  ...
  if(failSendD = 1 && time_tick_sleep > ticks_per_fail){
    //turn off watchdog timer while communicating
    wdt_disable();
  ...
  if(failSendA = 1 && time_tick_sleep > ticks_per_fail){
    //turn off watchdog timer while communicating
    wdt_disable();
...
void sendData(int levelState, String topic) {
  ...
  message = "";
  message += "SWT=";
  if(levelState == HIGH) {    message += "HIGH"; }
  else {                      message += "Nominal"; }
  message +=":T2=";
...
tick_socket = 0;
if (topic=="D"){ failSendD=1; }
if (topic=="A"){ failSendA=1; }
...
do {
    ...
    if (socket >= 0) {
      ...
      if (lte.socketConnect(socket, HOLOGRAM_URL, HOLOGRAM_PORT) == LTE_SHIELD_SUCCESS) {
        ...
        {
            if (topic=="D"){ failSendD=0; }
            if (topic=="A"){ failSendA=0; }          
...
ISR(WDT_vect) {
    time_tick_sleep ++; //for each Watchdog Interupt, adds 1 to the number of 8 second ticks counted so far
